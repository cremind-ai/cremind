...
- ``UNIQUE(profile, kind)`` on ``userdoc_sources`` — it is what makes "turn it
...
_NEW_TABLES = ("userdoc_sources", "userdoc_captions", "userdoc_vision_usage")
...
def test_userdocs_migration_sqlite(tmp_path: Path, monkeypatch) -> None:
...
def test_one_source_row_per_profile_and_kind(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(IntegrityError):
        with provider.sync_engine().begin() as c:
            for i in (1, 2):
                c.execute(text(
                    "INSERT INTO userdoc_sources (id,profile,kind,created_at,updated_at) "
                    f"VALUES ('s{i}','alice','local',0,0)"
                ))


def test_upsert_is_idempotent_and_keeps_profiles_apart(tmp_path: Path, monkeypatch) -> None:
    storage = UserDocsStorage(_upgraded(tmp_path, monkeypatch))
