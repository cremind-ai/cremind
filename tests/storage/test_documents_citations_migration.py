...
def test_upgrade_creates_the_registry(tmp_path: Path, monkeypatch) -> None:
    ...
    with provider.sync_engine().connect() as c:
        insp = inspect(c)
        assert "document_citations" in insp.get_table_names()
        cols = {col["name"]: col for col in insp.get_columns("document_citations")}
        assert set(cols) == {
        ...
        assert "ix_document_citations_profile_cite" in {
            i["name"] for i in insp.get_indexes("document_citations")
        }
        fks = {tuple(fk["constrained_columns"]): fk for fk in insp.get_foreign_keys("document_citations")}
...
def test_downgrade_removes_the_table(tmp_path: Path, monkeypatch) -> None:
    ...
    with provider.sync_engine().connect() as c:
        ...
        assert "document_citations" not in tables
