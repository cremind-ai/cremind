...
The revision is historical: it creates ``userdoc_research_jobs``, which
``20260928b_document_tables`` later renames to ``document_research_jobs``. So
the revision is checked at its own head against the table it really creates,
and the behaviour below runs on the same old install carried on to head. What
is worth pinning:
...
- the counters default to 0 at the DB level, so a row inserted by hand (or by
  a future raw-SQL path) starts undelivered-and-unchanged, not NULL — before
...
- a job written before the rename keeps its id, checkpoint and counters.
...
_REVISION = "20260927_userdocs_research"
_COLUMNS = {
...
def _job(c, jid, profile, conv, table="document_research_jobs"):
    c.execute(text(
        f"INSERT INTO {table} (id,profile,conversation_id,status,mode,domain,question,"
...
def _count(provider, where="1=1") -> int:
    with provider.sync_engine().connect() as c:
        return int(c.execute(text(f"SELECT COUNT(*) FROM document_research_jobs WHERE {where}")).scalar())


def _assert_jobs_table(c, table: str, prefix: str) -> None:
    insp = inspect(c)
    ...
    assert {f"ix_{prefix}_research_jobs_profile_created", f"ix_{prefix}_research_jobs_conversation"} == {
...
def test_upgrade_creates_the_jobs_table(tmp_path: Path, monkeypatch) -> None:
    ...
    with provider.sync_engine().connect() as c:
        _assert_jobs_table(c, "userdoc_research_jobs", "userdoc")
        assert c.execute(text("SELECT version_num FROM alembic_version")).scalar() == _REVISION

    with provider.sync_engine().begin() as c:
        _job(c, "j-old", "alice", "c1", table="userdoc_research_jobs")
        c.execute(text(
            "UPDATE userdoc_research_jobs SET rev=3, delivered_rev=2, tokens_in=40, "
            "state='{\"phase\": \"read\"}' WHERE id='j-old'"
        ))
    mig.upgrade("head")
    with provider.sync_engine().connect() as c:
        _assert_jobs_table(c, "document_research_jobs", "document")
        assert "userdoc_research_jobs" not in inspect(c).get_table_names()
        row = c.execute(text(
            "SELECT profile, conversation_id, rev, delivered_rev, tokens_in, state "
            "FROM document_research_jobs WHERE id='j-old'"
        )).one()
    assert tuple(row) == ("alice", "c1", 3, 2, 40, '{"phase": "read"}')


@pytest.mark.parametrize("target,table", [
    (_REVISION, "userdoc_research_jobs"),
    ("head", "document_research_jobs"),
])
def test_counters_default_to_zero(tmp_path: Path, monkeypatch, target, table) -> None:
    provider = _upgraded(tmp_path, monkeypatch, target)
    with provider.sync_engine().begin() as c:
        _job(c, "j1", "alice", "c1", table=table)
    with provider.sync_engine().connect() as c:
        row = c.execute(text(
            "SELECT rev, delivered_rev, tokens_in, tokens_out, budget, elapsed_s "
            f"FROM {table} WHERE id='j1'"
        )).fetchone()
