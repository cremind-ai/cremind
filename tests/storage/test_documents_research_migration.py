...
def _job(c, jid, profile, conv):
    c.execute(text(
        "INSERT INTO document_research_jobs (id,profile,conversation_id,status,mode,domain,question,"
...
def test_upgrade_creates_the_jobs_table(tmp_path: Path, monkeypatch) -> None:
    ...
    with provider.sync_engine().connect() as c:
        insp = inspect(c)
        assert "document_research_jobs" in insp.get_table_names()
        cols = {col["name"]: col for col in insp.get_columns("document_research_jobs")}
        assert set(cols) == {
        ...
        assert {"ix_document_research_jobs_profile_created", "ix_document_research_jobs_conversation"} <= {
            i["name"] for i in insp.get_indexes("document_research_jobs")
        }
        fks = {tuple(fk["constrained_columns"]): fk for fk in insp.get_foreign_keys("document_research_jobs")}
...
def test_counters_default_to_zero(tmp_path: Path, monkeypatch) -> None:
    ...
    with provider.sync_engine().connect() as c:
        row = c.execute(text(
            ...
            "FROM document_research_jobs WHERE id='j1'"
        )).fetchone()
...
def test_downgrade_removes_the_table(tmp_path: Path, monkeypatch) -> None:
    ...
    with provider.sync_engine().connect() as c:
        ...
        assert "document_research_jobs" not in tables
