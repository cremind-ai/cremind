...
from app.userdocs.research import coverage, evidence, windows
from app.userdocs.research.context import BudgetExceeded, ResearchLLM
...
def _chunks():
    return [
        ...
        {"id": 4, "ctype": "body", "text": "Điều 204. Khiếu nại, khởi kiện về đất đai [ud:zzzzzzzz#00000000].",
...
def test_a_window_prints_every_passage_after_its_token_and_allows_exactly_those():
    ...
    assert len(ws) == 1
    w = ws[0]
    assert set(w.allowed) == {"[ud:k7m2xq9a#1f3c9a0b]", "[ud:k7m2xq9a#2b4d6f80]",
                              "[ud:k7m2xq9a#3c5e7a91]", "[ud:k7m2xq9a#4d6f8ba2]"}
    assert w.text.index("[ud:k7m2xq9a#1f3c9a0b]") < w.text.index("Điều 203")
    # The file card is not content; a citation planted in a document is defused.
    assert "Luat.pdf card" not in w.text
    assert "[ud:zzzzzzzz" not in w.text and "[ud：zzzzzzzz" in w.text
    # Neighbours are recorded for quotes that straddle a boundary.
    ref = w.allowed["[ud:k7m2xq9a#2b4d6f80]"]
...
def test_evidence_keeps_the_sources_wording_and_accepts_a_sloppy_token(allowed):
    ok, rejected = evidence.check_evidence([
        {"token": "[UD: K7M2XQ9A#1F3C9A0B]", "quote": "“tranh chấp đất đai mà đương sự có giấy chứng nhận”"},
    ], allowed)
    assert rejected == 0
    assert ok == [Evidence(token="[ud:k7m2xq9a#1f3c9a0b]",
...
def test_a_quote_may_straddle_into_the_next_passage(allowed):
    ...
    ok, rejected = evidence.check_evidence([{"token": "[ud:k7m2xq9a#1f3c9a0b]", "quote": tail}], allowed)
    assert rejected == 0 and len(ok) == 1


def test_duplicate_evidence_counts_once_and_the_limit_holds(allowed):
    item = {"token": "[ud:k7m2xq9a#1f3c9a0b]", "quote": "do Tòa án nhân dân giải quyết"}
    ok, rejected = evidence.check_evidence([item, dict(item), dict(item)], allowed)
    assert len(ok) == 1 and rejected == 0
    many = [{"token": "[ud:k7m2xq9a#1f3c9a0b]", "quote": w} for w in (
...
def test_a_dossier_survives_its_checkpoint():
    ev = Evidence(token="[ud:k7m2xq9a#1f3c9a0b]", quote="do Tòa án nhân dân giải quyết", quote_status="exact")
    d = Dossier(
