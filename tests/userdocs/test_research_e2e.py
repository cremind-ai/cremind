...
from app.userdocs.query.engine import QueryEngine  # noqa: E402
from app.userdocs.research import jobs  # noqa: E402
from app.userdocs.research.types import COMPLETE, NEEDS_CONFIRMATION  # noqa: E402
...
def test_compile_a_folder_through_the_agents_tool(world):
    ...
    first = _call(tool.UserDocumentsResearchTool, question=QUESTION, mode="compile", domain="financial",
    ...
    second = _call(tool.UserDocumentsResearchTool, continue_job=job_id, answers={"confirm": True})
    text = _text(second)
    ...
    printed = set(__import__("re").findall(r"\[ud:[0-9a-z]{8}#[0-9a-f]{8}\]", text))
    assert printed
    rows = get_userdocs_citations_storage().rows_for_cite_ids("alice", {t[4:12] for t in printed})
    ...
    assert {t[4:12] for t in printed} <= folder_ids
    ...
    assert world.usage.calls and all(
        r["source_kind"] == "userdocs" and r["tool_id"] == "user_documents"
        for c in world.usage.calls for r in c["records"])

    # 3. Later pages of the dossier, through read.
    page2 = _call(tool.UserDocumentsReadTool, file=f"research:{job_id}", page=2)
    assert "research" in _text(page2).lower()

    # 4. Bob cannot see alice's job.
    stolen = _call(tool.UserDocumentsResearchTool, profile="bob", context="c-bob", continue_job=job_id)
