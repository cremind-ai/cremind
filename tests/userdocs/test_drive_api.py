"""API: Google Drive as a User Document Search source.
...
from app.api import userdocs_files as files_api  # noqa: E402
...
def _call(path, method, **req):
    return asyncio.run(_route(api.get_userdocs_routes(), path, method)(_Req(**req)))
...
def env(tmp_path: Path, monkeypatch):
    ...
    rows: dict[str, str] = {"userdocs.allowed": "true"}
...
def _put(profile: str, body: dict[str, Any]):
    return _call("/api/userdocs/settings", "PUT", username=profile, body=body)
...
@pytest.fixture
def svc(monkeypatch):
    import app.userdocs.service as service_module
...
def test_control_passes_source_and_drops_everything_else(svc):
    resp = _call("/api/userdocs/control", "POST", username="dog",
                 body={"action": "sync_now", "source": "drive", "profile": "cat"})
    assert resp.status_code == 202
    assert svc.calls == [("dog", "sync_now", {"source": "drive"})]

    bad = _call("/api/userdocs/control", "POST", body={"action": "rescan", "source": "dropbox"})
...
def test_files_source_filter(svc, query, expected):
    resp = _call("/api/userdocs/files", "GET", query=query)
    assert resp.status_code == 200
    assert svc.calls[-1][2]["source"] == expected


def test_files_rejects_an_unknown_source(svc):
    assert _call("/api/userdocs/files", "GET", query="source=dropbox").status_code == 400
    assert svc.calls == []
...
def _folders(profile="dog", query=""):
    return _call("/api/userdocs/drive/folders", "GET", username=profile, query=query)
...
def files_env(tmp_path, monkeypatch):
    from tests.userdocs._citations_env import build, close
...
def _files(path: str, fid: str, *, username="alice", query=""):
    return asyncio.run(_route(files_api.get_userdocs_files_routes(), path, "GET")(
...
def test_raw_proxies_a_drive_file_through_the_callers_link(files_env):
    ...
    resp = _files("/api/userdocs/files/{fid}/raw", rec["cite_id"])
    assert resp.status_code == 200
...
def test_a_drive_image_thumbnail_is_made_from_the_fetched_bytes(files_env):
    ...
    resp = _files("/api/userdocs/files/{fid}/thumbnail", rec["cite_id"], query="size=90")
    ...
    again = _files("/api/userdocs/files/{fid}/thumbnail", rec["cite_id"], query="size=90")
...
def test_a_hidden_drive_is_not_viewable(files_env):
    ...
    for path in ("/api/userdocs/files/{fid}/raw", "/api/userdocs/files/{fid}/text"):
        resp = _files(path, rec["cite_id"])
        assert resp.status_code == 404, path
        assert _body(resp)["error"] == "NotFound"
    assert _files("/api/userdocs/files/{fid}/text", folder["cite_id"]).status_code == 404
    assert files_env.made == []
    # Local files stay viewable while Drive is held.
    assert _files("/api/userdocs/files/{fid}/text", files_env.law["cite_id"]).status_code == 200

    files_env.db.update_source_state("drive", state="live", reason=None)
    files_env.spec["content"] = _content(b"%PDF")
    assert _files("/api/userdocs/files/{fid}/raw", rec["cite_id"]).status_code == 200


def test_another_profile_cannot_reach_a_drive_file(files_env):
    rec = _drive_row(files_env, "Drive/plan.pdf", "plan.pdf")
    for path in ("/api/userdocs/files/{fid}/raw", "/api/userdocs/files/{fid}/thumbnail"):
