...
from app.userdocs.research import compile as C
...
TOKEN_RE = re.compile(r"(\[ud:[0-9a-z]{8}#[0-9a-f]{8}\])[^\n]*\n(.*?)(?=\n\[ud:|\n<<<|\Z)", re.S)
...
class FakeModel:
    ...
    def summarize_table(self, user, tool):
        shown = re.findall(r'source (\[ud:[0-9a-z]{8}#[0-9a-f]{8}\]) "([^"]+)"', user)
...
def test_map_prompts_carry_the_window_as_untrusted_data(corpus):
    ...
    assert set(re.findall(r"\[ud:([0-9a-z]{8})#", q3)) == {corpus.q3["cite_id"]}
...
def test_values_merge_across_formats_and_periods_sort_by_time():
    ...
    rows = [{"cells": {"period": p, "metric": "revenue", "value": v},
             "evidence": [{"token": f"[ud:aaaaaaaa#{i:08x}]", "quote": "q", "quote_status": "exact"}]}
...
def test_the_csv_defuses_formulas_but_keeps_numbers():
    table = CompiledTable(columns=["item", "value"], rows=[
        {"item": Cell("=HYPERLINK(\"http://x\")", ["[ud:aaaaaaaa#00000001]"]), "value": Cell("-1,200")},
    ])
    out = artifacts.table_csv(table, {"aaaaaaaa": "Docs/a.csv"})
    row = list(csv.reader(out.splitlines()))[1]
    assert row == ["'=HYPERLINK(\"http://x\")", "-1,200", "[ud:aaaaaaaa#00000001] Docs/a.csv"]
