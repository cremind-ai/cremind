"""Backend-neutral pieces of the userdocs vector layer.
...
def test_collection_name_shape():
    ...
    assert name == f"ud_{v.profile_tag(UID)}_a1b2c3_me5_multilingual-e5-base_768_p{PROMPT_SCHEME}"
...
def test_parse_collection_name_round_trips_and_ignores_others():
    ...
    for other in ("cremind_docs", "memory_admin", "ud_nothex_ab_x_1_p1", "", "ud_"):
...
def test_source_codes_cover_the_settings_source_kinds():
    from app.userdocs.settings import SOURCE_KINDS
...
class _OldAdapter(VectorStoreBase):
    """An adapter written before the userdocs primitives existed."""
