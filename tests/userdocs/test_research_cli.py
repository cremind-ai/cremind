"""`cremind userdocs research …` — deep-research jobs from the terminal.

The commands import their client functions inside the body, so the client is
patched in ``app.cli.client.userdocs`` and nothing reaches the network. What
...
def client(monkeypatch):
    ...
    import app.cli.client.userdocs as c
    import app.cli.commands.userdocs as cmd
...
def test_run_flags_become_the_request_body(client):
    result = _run("userdocs", "research", "run", "Compile the business results",
                  "--mode", "compile", "--domain", "financial",
                  "--folder", "MKT-report", "--folder", "Sales", "--file", "k7m2xq9a",
                  "--reference-folder", "Luat", "--reference-file", "[ud:abcd2345]", "--wait", "30")
    ...
    assert body == {
        ...
        "reference_scope": {"folder": ["Luat"], "file_ids": ["[ud:abcd2345]"]},
        "wait": 30,
    }
...
def test_run_without_flags_leaves_the_defaults_to_the_server(client):
    result = _run("userdocs", "research", "run", "Why?")
    assert result.exit_code == 0, result.output
...
def test_follow_prints_progress_then_the_text_and_exits_by_outcome(client, status, code):
    ...
    result = _run("userdocs", "research", "run", "q", "--follow")
...
def test_a_question_prints_how_to_answer_it(client):
    ...
    result = _run("userdocs", "research", "status", "job1")
...
def test_busy_names_the_running_job(client, monkeypatch):
    ...
    result = _run("userdocs", "research", "run", "q")
...
def test_status_prints_one_page(client):
    ...
    result = _run("userdocs", "research", "status", "job1", "--page", "2")
    assert result.exit_code == 0, result.output
...
def test_status_all_pages_prints_every_page(client):
    ...
    result = _run("userdocs", "research", "status", "job1", "--all-pages")
    assert result.exit_code == 0, result.output
    out = result.stdout
...
def test_page_and_all_pages_are_exclusive(client):
    result = _run("userdocs", "research", "status", "job1", "--page", "2", "--all-pages")
    assert result.exit_code == 1
    assert client["calls"] == []
...
def test_status_follow_keeps_the_page(client):
    ...
    result = _run("userdocs", "research", "status", "job1", "--page", "2", "--follow")
    assert result.exit_code == 0, result.output
...
def test_json_prints_the_whole_answer(client):
    ...
    result = _run("--json", "userdocs", "research", "status", "job1")
    assert result.exit_code == 0, result.output
...
def test_continue_parses_answers(client):
    ...
    result = _run("userdocs", "research", "continue", "job1", "--answer", "edition=k7m2xq9a",
...
def test_continue_without_answers_resumes(client):
    result = _run("userdocs", "research", "continue", "job1", "--follow")
    assert result.exit_code == 0, result.output
...
def test_a_malformed_answer_sends_nothing(client, bad):
    result = _run("userdocs", "research", "continue", "job1", "--answer", bad)
    assert result.exit_code == 1
...
def test_cancel_prints_the_text_and_succeeds(client):
    ...
    result = _run("userdocs", "research", "cancel", "job1")
    assert result.exit_code == 0, result.output
...
def test_list_prints_one_row_per_job(client):
    ...
    result = _run("userdocs", "research", "list", "--limit", "5")
    assert result.exit_code == 0, result.output
...
def test_list_empty(client):
    result = _run("userdocs", "research", "list")
...
def test_the_status_sets_mirror_the_server():
    import app.cli.commands.userdocs as cmd
    from app.userdocs.research import types as T
...
def test_the_client_builds_the_documented_requests(monkeypatch):
    ...
    asyncio.run(go())
    assert seen == [
        ("POST", "/api/userdocs/research", {"question": "q", "wait": 5}),
        ("GET", "/api/userdocs/research/job1", {"page": 2, "wait": 20}),
        ("GET", "/api/userdocs/research/job1", None),
        ("POST", "/api/userdocs/research/job1/continue", {"answers": {"edition": "k7m2xq9a"}}),
        ("POST", "/api/userdocs/research/a%2Fb/cancel", {}),
        ("GET", "/api/userdocs/research", {"limit": 7}),
    ]
...
def test_the_cli_imports_no_server_module():
    ...
    code = (
        "import sys, app.cli.main, app.cli.commands.userdocs, app.cli.client.userdocs\n"
        "bad = ('app.server', 'app.api', 'app.tools', 'app.agent', 'app.skills', 'app.events', "
        "'app.documents', 'app.channels', 'app.databases', 'app.storage', 'app.userdocs')\n"
