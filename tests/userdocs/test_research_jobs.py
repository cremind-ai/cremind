"""The research job runner (``app.userdocs.research.jobs``).
...
import app.storage.userdocs_citations_storage as cit_module  # noqa: E402
import app.storage.userdocs_research_storage as research_storage_module  # noqa: E402
import app.storage.userdocs_storage as uds_module  # noqa: E402
...
from app.storage.userdocs_citations_storage import UserDocCitationsStorage  # noqa: E402
from app.storage.userdocs_research_storage import UserDocResearchStorage  # noqa: E402
from app.storage.userdocs_storage import UserDocsStorage  # noqa: E402
from app.userdocs.research import context as C  # noqa: E402
from app.userdocs.research import jobs  # noqa: E402
from app.userdocs.research.context import (  # noqa: E402
    BudgetExceeded,
    NeedsInput,
    ResearchSpec,
)
from app.userdocs.research.errors import (  # noqa: E402
    InvalidRequest,
    JobNotFound,
    ResearchBusy,
    ResearchUnavailable,
)
from app.userdocs.research.types import (  # noqa: E402
    CANCELLED,
    COMPLETE,
    FAILED,
    INTERRUPTED,
...
TABLES = (
    ...
    "userdoc_sources", "userdoc_captions", "userdoc_vision_usage", "userdoc_citations",
...
def env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uds_module, "_instance", UserDocsStorage(provider))
    monkeypatch.setattr(cit_module, "_instance", UserDocCitationsStorage(provider))
    store = UserDocResearchStorage(provider)
...
def test_bad_requests_and_an_unavailable_index(env):
    ...
    env.engine_error = ResearchUnavailable("User Document Search is off.", status_code="disabled")
...
def test_usage_is_recorded_per_phase_against_the_conversation(env):
    ...
    for r in records:
        assert (r["source_kind"], r["tool_id"], r["label"]) == ("userdocs", "user_documents",
...
def test_the_activity_panel_follows_the_job(env, monkeypatch):
    ...
    from app.userdocs.research.context import ProgressSink
...
def test_retention_keeps_the_newest_and_never_a_running_or_waiting_job(env):
    ...
    with env.provider.sync_engine().begin() as c:
        c.execute(text("UPDATE userdoc_research_jobs SET created_at = CAST(SUBSTR(id, 4) AS INTEGER) "
                       "WHERE id LIKE 'job%'"))
        c.execute(text("UPDATE userdoc_research_jobs SET created_at = -1 WHERE id = 'waiting'"))
...
def test_purge_deletes_the_profiles_jobs_and_artifacts_and_stops_a_running_one(env):
    ...
    async def scenario():
        ...
        # The User Document Search engine purges from its own thread.
...
def test_cancel_profile_jobs_stops_only_that_profile(env):
    ...
    async def scenario():
        ...
        n = await jobs.cancel_profile_jobs("alice", "User Document Search was turned off")
    ...
    assert n == 1
    assert a_view.status == CANCELLED and a_view.error == "User Document Search was turned off"
