...
from app.userdocs.chunking import chunk_blocks, diff_chunks, make_file_card, make_folder_card
from app.userdocs.cite import TOKEN_RE
from app.userdocs.discovery.walker import path_hash
from app.userdocs.index import IndexDB
from app.userdocs.query import ReadError
from app.userdocs.query import vector as vector_module
from app.userdocs.query.engine import QueryEngine
from app.userdocs.query.filters import Filters, parse_filters
from app.userdocs.query.render import RenderContext, render_read, render_search
from app.userdocs.textnorm import fold
from app.userdocs.types import Block
from tests.userdocs.legal_samples import vietnamese_law_blocks
...
def test_a_chunk_token_opens_around_its_passage(docs):
    ...
    out = _engine(docs).read(f"[ud:{docs.pdf['cite_id']}#{target['text_hash'][:8]}]")
...
def test_search_render_shrinks_but_keeps_every_token_whole(docs):
    ...
    assert small.text.startswith("[User Documents · search")
