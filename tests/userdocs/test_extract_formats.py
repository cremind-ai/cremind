...
from app.userdocs.extract.dispatch import DEFAULT_LIMITS, extract_any
from app.userdocs.extract.formats._base import group_rows
from app.userdocs.extract.formats.legacy import doc_sections, parse_piece_table, piece_text
from app.userdocs.extract.formats.rtf import rtf_to_text
...
from app.userdocs import types
if True:
...
def test_python_blocks_follow_definitions() -> None:
    ...
    meta = r.blocks[0]
    assert meta.role == "meta" and meta.text == "imports: os, sys, app.userdocs, json"
...
def test_pdf_damaged_page_keeps_the_others(monkeypatch) -> None:
    from app.userdocs.extract.formats import pdf as pdf_mod
...
def test_piece_table_rejects_garbage() -> None:
    from app.userdocs.extract.formats._base import LegacyFormatError
...
def test_heic_with_and_without_pillow_heif(monkeypatch) -> None:
    ...
    from app.userdocs.extract.formats import image as image_mod
...
def test_failure_after_blocks_keeps_them_as_partial(monkeypatch) -> None:
    from app.userdocs.extract.formats import text as text_mod
