"""The per-profile index database (``app.userdocs.index``).
...
from app.userdocs.index import db as db_mod
...
from app.userdocs.types import Chunk, ChunkDiff, ManifestRow
...
def test_index_path_lives_under_the_system_dir(tmp_path, monkeypatch):
    ...
    assert index_path("abc") == os.path.join(str(tmp_path), "storage", "userdocs", "abc", "index.db")
    for bad in ("", "..", "a/b", "a\\b"):
