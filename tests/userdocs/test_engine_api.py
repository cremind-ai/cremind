...
from app.userdocs.errors import EngineError, NotFound  # noqa: E402
...
def test_no_engine_is_a_503(monkeypatch):
    ...
    status, body = _call("/api/userdocs/files", "GET")
    assert status == 503
...
def test_errors_keep_their_status_and_fields(monkeypatch):
    ...
    status, body = _call("/api/userdocs/control", "POST", body={"action": "rebuild"})
    assert status == 409
    assert body["error"] == "ConfirmationRequired" and body["confirm"] == "tok"
    status, body = _call("/api/userdocs/files/{fid}", "GET", path_params={"fid": "nope1234"})
    assert status == 404
...
def test_the_profile_is_always_the_callers(monkeypatch):
    stub = _StubService()
    monkeypatch.setattr(svc_module, "_service", stub)
    status, _ = _call("/api/userdocs/control", "POST", username="bob",
    ...
    _call("/api/userdocs/files", "GET", username="bob", query="status=error&limit=5")
...
def test_bad_query_numbers_are_a_400(monkeypatch):
    ...
    status, body = _call("/api/userdocs/files", "GET", query="limit=lots")
    assert status == 400
