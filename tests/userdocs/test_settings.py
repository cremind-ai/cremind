"""User Document Search settings: root validation, option hygiene, change plans.
...
def test_admin_policy_round_trips_and_rejects_bad_input(monkeypatch):
    ...
    assert store.rows["userdocs.allowed"] == "true"
    ...
    assert "userdocs.workers" not in store.rows
