...
from app.api import userdocs_files as api  # noqa: E402
...
def _route(path: str, method: str):
    for r in api.get_userdocs_files_routes():
...
@pytest.mark.parametrize("path,method", [
    ("/api/userdocs/citations/resolve", "POST"),
    ("/api/userdocs/files/{fid}/text", "GET"),
    ("/api/userdocs/files/{fid}/raw", "GET"),
    ("/api/userdocs/files/{fid}/thumbnail", "GET"),
])
...
def test_resolve(env):
    ...
    resp = _call("/api/userdocs/citations/resolve", "POST",
                 body={"conversation_id": "c-web", "tokens": [token, "[ud:zzzzzzzz]", "junk"]})
    ...
    assert items["[ud:zzzzzzzz]"]["status"] == "invalid"
    assert items["junk"]["status"] == "invalid"
    # Without a conversation, a registered token is "elsewhere".
    items = _json(_call("/api/userdocs/citations/resolve", "POST", body={"tokens": [token]}))["items"]
...
def test_resolve_is_profile_scoped(env):
    ...
    resp = _call("/api/userdocs/citations/resolve", "POST", username="bob",
                 body={"conversation_id": "c-web", "tokens": [token]})
    assert resp.status_code == 404
    items = _json(_call("/api/userdocs/citations/resolve", "POST", username="bob",
...
@pytest.mark.parametrize("body", [
    {"tokens": "[ud:k7m2xq9a]"},
    {"tokens": [1, 2]},
    {"tokens": ["[ud:k7m2xq9a]"] * 101},
    {"tokens": [], "conversation_id": 7},
])
def test_resolve_validates(env, body):
    assert _call("/api/userdocs/citations/resolve", "POST", body=body).status_code == 400
...
def test_text_around_a_chunk(env):
    ...
    resp = _call("/api/userdocs/files/{fid}/text", "GET", path_params={"fid": law["cite_id"]},
    ...
    body = _json(_call("/api/userdocs/files/{fid}/text", "GET", path_params={"fid": law["cite_id"]},
                       query="pages=3"))
...
def test_text_errors(env):
    law = env.law
    fid = {"fid": law["cite_id"]}
    assert _call("/api/userdocs/files/{fid}/text", "GET", path_params=fid,
                 query="chunk=0badc0de").status_code == 404
    assert _call("/api/userdocs/files/{fid}/text", "GET", path_params=fid,
                 query="chunk=nothex!!").status_code == 400
    assert _call("/api/userdocs/files/{fid}/text", "GET", path_params=fid,
                 query="pages=a-b").status_code == 400
    assert _call("/api/userdocs/files/{fid}/text", "GET",
...
def test_another_profiles_fid_is_a_404_everywhere(env):
    ...
    for path in ("/api/userdocs/files/{fid}/text", "/api/userdocs/files/{fid}/raw",
                 "/api/userdocs/files/{fid}/thumbnail"):
...
def test_raw_serves_the_file(env):
    resp = _call("/api/userdocs/files/{fid}/raw", "GET", path_params={"fid": env.law["cite_id"]})
    assert resp.status_code == 200
...
def test_raw_refuses_a_path_outside_the_root(env, tmp_path):
    ...
    resp = _call("/api/userdocs/files/{fid}/raw", "GET", path_params={"fid": env.note["cite_id"]})
...
def test_raw_never_serves_active_content_inline(env):
    ...
    resp = _call("/api/userdocs/files/{fid}/raw", "GET", path_params={"fid": rec["cite_id"]})
    assert resp.status_code == 200
...
def test_thumbnail_is_made_in_memory_and_cached(env, monkeypatch):
    ...
    resp = _call("/api/userdocs/files/{fid}/thumbnail", "GET", path_params={"fid": rec["cite_id"]},
                 query="size=128")
    ...
    again = _call("/api/userdocs/files/{fid}/thumbnail", "GET", path_params={"fid": rec["cite_id"]},
                  query="size=128")
    assert again.status_code == 200 and again.body == resp.body


def test_thumbnail_only_for_images(env):
    resp = _call("/api/userdocs/files/{fid}/thumbnail", "GET", path_params={"fid": env.law["cite_id"]})
