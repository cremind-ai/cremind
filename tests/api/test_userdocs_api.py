"""API: User Document Search settings, the admin gate, and folder browsing.
...
from app.api import userdocs as api  # noqa: E402
...
def _route(path: str, method: str):
    for r in api.get_userdocs_routes():
...
def _allow(env):
    env.rows["userdocs.allowed"] = "true"
...
@pytest.mark.parametrize("path,method", [
    ("/api/userdocs/status", "GET"),
    ("/api/userdocs/settings", "GET"),
    ("/api/userdocs/settings", "PUT"),
    ("/api/userdocs/validate-root", "POST"),
    ("/api/userdocs/browse", "GET"),
    ("/api/userdocs/admin", "GET"),
])
...
def test_the_gate_is_admin_only(env):
    assert _call("/api/userdocs/admin", "GET", username="dog").status_code == 403
    resp = _call("/api/userdocs/admin", "PUT", username="dog", body={"allowed": True})
    assert resp.status_code == 403
    assert "userdocs.allowed" not in env.rows
...
def test_admin_can_allow_and_set_budgets(env):
    resp = _call("/api/userdocs/admin", "PUT", body={"allowed": True, "storage_budget_mb": 4096})
    assert resp.status_code == 200
    assert _body(resp)["policy"]["allowed"] is True
    assert env.rows["userdocs.storage_budget_mb"] == "4096"

    bad = _call("/api/userdocs/admin", "PUT", body={"workers": 0})
...
def test_the_admin_overview_never_shows_other_profiles_paths(env):
    ...
    body = _body(_call("/api/userdocs/admin", "GET"))
...
def test_a_profile_cannot_enable_before_the_admin_allows_it(env):
    resp = _call("/api/userdocs/settings", "PUT", username="dog", body={"enabled": True})
    assert resp.status_code == 409
...
def test_profiles_only_ever_see_their_own_settings(env):
    _allow(env)
    _call("/api/userdocs/settings", "PUT", username="dog",
          body={"enabled": True, "root_path": str(env.wd / "Reports")})
    mine = _body(_call("/api/userdocs/settings", "GET", username="admin"))
    assert mine["local"]["enabled"] is False
    assert mine["local"]["root_path"] is None
    status = _body(_call("/api/userdocs/status", "GET", username="admin"))
    assert status["enabled"] is False


def test_a_member_cannot_index_outside_the_working_directory(env):
    _allow(env)
    resp = _call("/api/userdocs/settings", "PUT", username="dog",
...
def test_nobody_can_index_the_system_folder(env):
    _allow(env)
    resp = _call("/api/userdocs/settings", "PUT", username="admin",
...
def test_moving_the_folder_needs_the_matching_confirmation(env):
    _allow(env)
    _call("/api/userdocs/settings", "PUT", username="dog", body={"enabled": True})
    move = {"root_path": str(env.wd / "Reports")}

    first = _call("/api/userdocs/settings", "PUT", username="dog", body=move)
    ...
    other = _call("/api/userdocs/settings", "PUT", username="dog", body=elsewhere)
    assert other.status_code == 409
    assert _body(other)["confirm"] != token
    assert env.storage.get_source("dog", "local")["root_path"] == os.path.realpath(env.wd)

    applied = _call("/api/userdocs/settings", "PUT", username="dog", body={**move, "confirm": token})
...
def test_disable_keeps_the_index_unless_asked(env):
    ...
    keep = _call("/api/userdocs/settings", "PUT", username="dog", body={"enabled": False})
    assert keep.status_code == 200
    assert env.purged == []

    _call("/api/userdocs/settings", "PUT", username="dog", body={"enabled": True})
    ask = _call("/api/userdocs/settings", "PUT", username="dog",
                body={"enabled": False, "delete_index": True})
    assert ask.status_code == 409
    token = _body(ask)["confirm"]
    done = _call("/api/userdocs/settings", "PUT", username="dog",
...
def test_delete_index_is_refused_while_staying_enabled(env):
    _allow(env)
    _call("/api/userdocs/settings", "PUT", username="dog", body={"enabled": True})
    resp = _call("/api/userdocs/settings", "PUT", username="dog", body={"delete_index": True})
    assert resp.status_code == 400
...
def test_status_reports_the_suspension_reason(env):
    ...
    _call("/api/userdocs/settings", "PUT", username="dog", body={"enabled": True})
    env.embedding["on"] = False
    snap = _body(_call("/api/userdocs/status", "GET", username="dog"))
    assert (snap["state"], snap["reason"], snap["tool_mode"]) == (
        "suspended", "embedding_off", "lexical_only",
    )
    env.rows["userdocs.allowed"] = "false"
    snap = _body(_call("/api/userdocs/status", "GET", username="dog"))
...
def test_browse_hides_credentials_hidden_folders_and_the_system_folder(env):
    body = _body(_call("/api/userdocs/browse", "GET", username="dog"))
...
def test_a_member_cannot_browse_outside_the_working_directory(env):
    resp = _call("/api/userdocs/browse", "GET", username="dog",
                 query=f"path={env.outside}")
    assert resp.status_code == 403
    assert _call("/api/userdocs/browse", "GET", username="admin",
