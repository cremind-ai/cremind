"""`/api/documentation-search/research` — the REST face of deep-research jobs.

The routes are thin over :mod:`app.documents.research.jobs` and
:mod:`app.documents.research.render`, so both are replaced by fakes (through
...
from app.api import documents_research as api  # noqa: E402
from app.documents.research import types as T  # noqa: E402
from app.documents.research.errors import JobNotFound, ResearchBusy, ResearchUnavailable  # noqa: E402
...
def env(monkeypatch):
    ...
    app = Starlette(routes=api.get_documents_research_routes(),
...
def test_start_is_a_202_for_the_callers_profile_with_no_conversation(env):
    resp = env.client.post("/api/documentation-search/research", headers=_as("alice"), json={
...
def test_start_defaults_to_analyze_general(env):
    resp = env.client.post("/api/documentation-search/research", headers=_as("alice"), json={"question": "Why?"})
...
def test_a_bad_body_is_a_400_and_starts_nothing(env, body):
    resp = env.client.post("/api/documentation-search/research", headers=_as("alice"), json=body)
...
def test_a_bad_scope_is_an_invalid_filter_naming_the_scope(env, field, value):
    resp = env.client.post("/api/documentation-search/research", headers=_as("alice"), json={"question": "q", field: value})
...
def test_busy_is_a_409_that_names_the_running_job(env):
    ...
    resp = env.client.post("/api/documentation-search/research", headers=_as("alice"), json={"question": "q"})
...
def test_unavailable_is_a_503(env):
    env.jobs.raise_on_start = ResearchUnavailable("Documentation search is off.", status_code="disabled")
    resp = env.client.post("/api/documentation-search/research", headers=_as("alice"), json={"question": "q"})
    assert resp.status_code == 503
    assert resp.json()["error"] == "DocumentsUnavailable"


def test_wait_is_capped_and_a_finished_job_is_collected(env):
    env.jobs.after_wait = T.COMPLETE
    resp = env.client.post("/api/documentation-search/research", headers=_as("alice"), json={"question": "q", "wait": 999})
    assert resp.status_code == 202
...
def test_unauthenticated_is_a_401(env):
    assert env.client.post("/api/documentation-search/research", json={"question": "q"}).status_code == 401
    assert env.client.get("/api/documentation-search/research").status_code == 401
...
def test_another_profiles_job_does_not_exist(env):
    ...
    for method, path in (("get", "/api/documentation-search/research/j-alice"),
                         ("post", "/api/documentation-search/research/j-alice/continue"),
                         ("post", "/api/documentation-search/research/j-alice/cancel")):
...
def test_get_renders_the_page_whole_and_counts_the_pages(env):
    ...
    resp = env.client.get("/api/documentation-search/research/j1?page=3", headers=_as("alice"))
...
def test_page_is_clamped_to_at_least_one(env, query, page):
    env.jobs.add("j1", "alice", T.COMPLETE)
    assert env.client.get(f"/api/documentation-search/research/j1?{query}", headers=_as("alice")).status_code == 200
...
def test_non_numeric_params_are_a_400(env):
    ...
    for query in ("page=last", "wait=long", "page=nan", "wait=inf"):
        resp = env.client.get(f"/api/documentation-search/research/j1?{query}", headers=_as("alice"))
...
def test_wait_on_a_running_job_is_clamped(env, query, expected):
    env.jobs.add("j1", "alice", T.RUNNING)
    env.client.get(f"/api/documentation-search/research/j1?{query}", headers=_as("alice"))
...
def test_a_finished_job_is_never_waited_on(env):
    ...
    env.client.get("/api/documentation-search/research/j1?wait=20", headers=_as("alice"))
...
def test_delivery_is_claimed_only_for_a_deliverable_state(env, status):
    ...
    resp = env.client.get("/api/documentation-search/research/j1", headers=_as("alice"))
    assert resp.status_code == 200
...
def test_looking_at_a_chat_job_from_the_terminal_does_not_steal_its_delivery(env):
    ...
    result; ``cremind docs research status`` on it must not claim it."""
...
def test_a_long_job_id_is_not_found_without_a_lookup(env):
    resp = env.client.get("/api/documentation-search/research/" + "a" * 100, headers=_as("alice"))
...
def test_list_is_the_callers_jobs_without_dossiers(env):
    ...
    resp = env.client.get("/api/documentation-search/research?limit=500", headers=_as("alice"))
    assert resp.status_code == 200
    jobs = resp.json()["jobs"]
...
def test_list_limit_defaults_and_rejects_words(env):
    env.client.get("/api/documentation-search/research", headers=_as("alice"))
    assert env.jobs.named("list")[-1][2] == 20
    assert env.client.get("/api/documentation-search/research?limit=all", headers=_as("alice")).status_code == 400
...
def test_continue_passes_the_answers_and_waits_while_running(env):
    ...
    env.jobs.after_wait = T.COMPLETE
    resp = env.client.post("/api/documentation-search/research/j1/continue", headers=_as("alice"), json={
...
def test_bad_answers_are_a_400(env, answers):
    env.jobs.add("j1", "alice", T.NEEDS_CLARIFICATION)
    resp = env.client.post("/api/documentation-search/research/j1/continue", headers=_as("alice"), json={"answers": answers})
    assert resp.status_code == 400
    assert resp.json()["error"] == "InvalidRequest"
...
def test_cancel_answers_the_cancelled_job_and_collects_it(env):
    env.jobs.add("j1", "alice", T.RUNNING)
    resp = env.client.post("/api/documentation-search/research/j1/cancel", headers=_as("alice"))
    assert resp.status_code == 200
...
def test_the_routes_and_their_registration_order():
    routes = {(r.path, m) for r in api.get_documents_research_routes() for m in (r.methods or ()) if m != "HEAD"}
    assert routes == {
        ("/api/documentation-search/research", "POST"),
        ("/api/documentation-search/research", "GET"),
        ("/api/documentation-search/research/{job_id}", "GET"),
        ("/api/documentation-search/research/{job_id}/continue", "POST"),
        ("/api/documentation-search/research/{job_id}/cancel", "POST"),
    }
    import app.api as api_pkg

    source = inspect.getsource(api_pkg.get_api_routes)
    assert "get_documents_research_routes()" in source
    # Registered with the query routes, ahead of the settings/engine routes.
    assert source.index("get_documents_research_routes()") < source.index("get_documents_routes()")
