"""`cremind docs drive …` and the `--source` flags — Drive from the terminal.

The client functions are patched in ``app.cli.client.docs``, so nothing
...
def test_status_json_is_the_source_and_the_link(calls):
    result = _run("--json", "docs", "drive", "status")
    assert result.exit_code == 0, result.output
...
def test_status_of_a_held_drive_says_when_the_index_goes(calls):
    ...
    line = _run("docs", "drive", "status").stdout
...
def test_enable_without_folders_leaves_them_alone(calls):
    assert _run("docs", "drive", "enable").exit_code == 0
    assert calls["put"] == [{"kind": "drive", "enabled": True}]
...
def test_enable_explains_a_whole_drive_account(calls):
    ...
    assert "cremind docs drive folders list" in result.stderr
...
def test_disable_without_yes_sends_nothing_and_exits_2(calls):
    result = _run("docs", "drive", "disable")
...
def test_disable_with_yes_answers_the_servers_confirmation(calls):
    ...
    result = _run("docs", "drive", "disable", "--yes")
    assert result.exit_code == 0, result.output
    assert calls["put"] == [
...
def test_sync_reads_changes_and_full_relists(calls):
    assert _run("docs", "drive", "sync").exit_code == 0
    assert _run("docs", "drive", "sync", "--full").exit_code == 0
...
def test_folders_list_marks_the_indexed_ones(calls):
    result = _run("docs", "drive", "folders", "list")
    assert result.exit_code == 0, result.output
...
def test_folders_list_takes_a_parent_link(calls):
    ...
    result = _run("docs", "drive", "folders", "list", "--parent",
...
def test_folders_list_json_is_the_listing(calls):
    result = _run("--json", "docs", "drive", "folders", "list")
...
def test_folders_set_replaces_the_list(calls):
    result = _run("docs", "drive", "folders", "set", "F1", "https://drive.google.com/drive/folders/F7")
    assert result.exit_code == 0, result.output
    assert calls["put"] == [{"kind": "drive", "options": {"include_folders": ["F1", "F7"]}}]


def test_folders_set_clear_is_the_only_empty_list(calls):
    assert _run("docs", "drive", "folders", "set", "--clear").exit_code == 0
    assert calls["put"] == [{"kind": "drive", "options": {"include_folders": []}}]
    assert _run("docs", "drive", "folders", "set").exit_code == 1
    assert _run("docs", "drive", "folders", "set", "F1", "--clear").exit_code == 1
...
def test_narrowing_the_folders_asks_first(calls):
    ...
    result = _run("docs", "drive", "folders", "set", "F1")
    assert result.exit_code == 2
...
def test_files_source(calls, flag, expected):
    assert _run("docs", "files", *flag).exit_code == 0
    assert calls["files"][0]["source"] == expected


def test_a_bad_source_fails_before_any_request(calls):
    result = _run("docs", "files", "--source", "dropbox")
    assert result.exit_code == 1
    assert calls["files"] == []
    assert _run("docs", "search", "x", "--source", "dropbox").exit_code == 1
    assert calls["query"] == []


def test_search_and_find_filter_by_source(calls):
    assert _run("docs", "search", "budget", "--source", "drive").exit_code == 0
    assert _run("docs", "find", "--source", "local", "--type", "pdf").exit_code == 0
    assert _run("docs", "search", "budget", "--source", "all").exit_code == 0
...
def test_deletions_and_retry_can_aim_at_drive(calls):
    assert _run("docs", "deletions", "confirm", "--source", "drive").exit_code == 0
    assert _run("docs", "deletions", "reject").exit_code == 0
    assert _run("docs", "retry", "--source", "drive", "--file-id", "k7m2xq9a").exit_code == 0
    assert calls["control"] == [
        ("confirm_deletions", {"source": "drive"}),
        ("reject_deletions", {}),
        ("retry_failed", {"source": "drive", "targets": ["k7m2xq9a"]}),
    ]
    assert _run("docs", "deletions", "confirm", "--source", "all").exit_code == 1
...
def test_summarize_snapshot_shows_drive_only_when_on():
    from app.cli.commands.docs import summarize_snapshot
