...
from app.documents.chunking import CHUNKER_VERSION, chunk_blocks, detect_legal_meta, diff_chunks, looks_legal
...
_PASSAGE_RE = re.compile(r"(\[doc:[0-9a-z]{8}#[0-9a-f]{8}\])[^\n]*\n(.*?)(?=\n\[doc:|\n###|\n<<<|\Z)", re.S)
...
def record_findings(_system: str, user: str) -> dict:
    ...
    out.append({"stance": "supports", "provision": "?", "text": "Invented.",
                "evidence": [{"token": "[doc:zzzzzzzz#00000000]", "quote": "anything at all goes here"}]})
