"""`cremind docs caption|identity|allow-in` — option saves from the terminal.
...
def test_caption_limits_are_one_put_with_only_the_given_fields(calls):
    result = _run("docs", "caption", "--daily-cap", "200", "--min-px", "512")
...
def test_consent_names_the_model_the_server_showed(calls):
    result = _run("docs", "caption", "--consent-vision")
    assert result.exit_code == 0, result.output
...
def test_revoke_and_consent_are_exclusive(calls):
    result = _run("docs", "caption", "--consent-vision", "--revoke-consent")
    assert result.exit_code == 1
    assert calls["control"] == []


def test_identity_lists_take_repeats_and_commas(calls):
    result = _run("docs", "identity", "--name", "Ann Nguyen, Nguyễn Thị An", "--name", "A. N.",
                  "--camera", "iPhone 14")
    assert result.exit_code == 0, result.output
...
def test_identity_empty_value_clears_and_no_flags_only_prints(calls):
    assert _run("docs", "identity", "--email", "").exit_code == 0
    assert calls["put"] == [{"kind": "local", "options": {"identity": {"emails": []}}}]
    calls["put"].clear()
    result = _run("docs", "identity")
    assert result.exit_code == 0, result.output
    assert calls["put"] == []
...
def test_allow_in_sends_only_the_switches_given(calls):
    result = _run("docs", "allow-in", "--channels", "--no-rooms")
    assert result.exit_code == 0, result.output
