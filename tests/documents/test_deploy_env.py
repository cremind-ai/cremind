...
from app.documents import deploy_env as de
...
# and `cremind docs status` warn from this block. It is read by configure
...
def _runtime(root: str | None = "/root/Documents", *, local_on: bool = True):
    from app.documents.runtime import ProfileRuntime
...
def test_configure_rechecks_the_container_mount(probe, monkeypatch, tmp_path):
    ...
    import app.storage.documents_storage as uds_storage
    ...
    monkeypatch.setattr(uds_storage, "get_documents_storage", lambda: _Storage())
