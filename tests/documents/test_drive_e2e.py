"""Google Drive as a Documentation search source, through the real engine.
...
from app.documents import types as t  # noqa: E402
from app.documents.sources import drive as drv  # noqa: E402
...
def test_a_drive_only_profile_searches_and_gets_the_tool(env, drive):
    from app.documents import gate
    ...
    assert gate.documents_tool_available("bob", None) is True
    assert gate.documents_tool_available("bob", {"source": "channel"}) is False
    ...
    assert gate.documents_tool_available("bob", None) is False
...
def test_reindex_and_retry_take_drive_paths_and_ids(env, drive):
    ...
    from app.documents.errors import NotEnabled, UnknownAction
...
def test_drive_photos_are_captioned_from_memory_and_rechecked_by_download(env, drive, vision, monkeypatch, tmp_path):
    from app.documents import runtime as rt_module
...
def test_drive_notes_say_what_a_drive_hold_means_for_an_answer():
    ...
    from app.documents.query.engine import drive_notes, status_notes
