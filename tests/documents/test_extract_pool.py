...
These spawn ``python -m app.documents.extract.worker`` for real (on Windows
...
from app.documents.extract.pool import ExtractorPool, wipe_stale_tmp
...
def test_unstartable_worker_is_reported(tmp_path, monkeypatch) -> None:
    import app.documents.extract.pool as pool_mod

    monkeypatch.setattr(pool_mod, "_WORKER_MODULE", "app.documents.extract.does_not_exist")
    with ExtractorPool(1, tmp_root=str(tmp_path / "t")) as p:
        r = p.extract(ExtractRequest(name="a.txt", kind="text", path=_text_file(tmp_path)))
