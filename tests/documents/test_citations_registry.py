...
from app.documents.cite import IssuedCitation  # noqa: E402
from app.documents.index import index_dir  # noqa: E402
...
def test_all_six_statuses(env):
    ...
    stale = "[doc:" + law["cite_id"] + "#deadbeef]"
    ...
    answer = (
        ...
        f"Old wording {stale}. Removed {gone_token}. Invented [doc:zzzzzzzz#00000000]."
    )
    ...
    assert statuses["[doc:zzzzzzzz#00000000]"] == "invalid"
...
def test_a_chunk_token_on_a_live_file_with_an_invented_hash_is_invalid(env):
    law = env.law
    meta = cit.finalize_citations("alice", "c-web", f"See [doc:{law['cite_id']}#0badc0de].")
    assert meta["items"][0]["status"] == "invalid"


def test_folder_tokens_resolve(env):
    folder = env.folder
    token = f"[doc:{folder['cite_id']}]"
...
def test_tolerant_parsing_strict_verification(env):
    ...
    sloppy = canonical.replace("[doc:", "【doc: ").replace("]", "】").upper().replace("DOC:", "doc:")
    ...
    # A pre-rename ud: token with a letter outside the cite alphabet parses,
    # and is invalid. Under doc: the same id is ordinary prose, not a token.
    meta = cit.finalize_citations("alice", "c-web", "Bad [ud:iiiiiiii#00000000].")
    assert meta["items"][0]["status"] == "invalid"
    assert meta["items"][0]["token"] == "[doc:iiiiiiii#00000000]"
    assert cit.finalize_citations("alice", "c-web", "Prose [doc:iiiiiiii#00000000].") is None
...
def test_issue_never_raises_into_the_tool(env, monkeypatch):
    import app.storage.documents_citations_storage as cit_storage
...
def test_non_canonical_tokens_are_not_registered(env):
    law = env.law
    cit.issue("alice", "c-web", [IssuedCitation(token="[doc:NOT-A-TOKEN]", cite_id=law["cite_id"])])
    cit.issue("alice", "c-web", [IssuedCitation(token=f"[doc:{law['cite_id']}]", cite_id="00000000")])
...
def test_finalization_failure_saves_the_message_without_citations(env, monkeypatch):
    ...
    assert cit.finalize_citations("alice", "c-web", "x [doc:zzzzzzzz]") is None
...
def test_a_held_drive_citation_shows_its_snapshot_not_the_live_index(env):
    ...
    from app.documents.types import ChunkDiff
