...
index is a real :class:`~app.documents.index.IndexDB`; the runtime is a stub
...
from app.documents.sources import drive as drv
from app.documents.sources.drive import DriveSource, drive_key, drive_rel_path, is_whole_drive
...
class StubRuntime:
    ...
    def index_content(self, *, row, name, rel, kind, mime, sha, size, result, status, reason, error, image,
        ...
        from app.documents.chunking import CHUNKER_VERSION, chunk_blocks, diff_chunks, make_file_card
        from app.documents.extract import EXTRACTOR_VERSION
        from app.documents.runtime import iso_local
...
def test_delete_source_kind_removes_one_profiles_drive_citations(tmp_path, monkeypatch):
    ...
    def row(token, kind):
        return {"token": token, "cite_id": token[5:13], "target": "file", "ref_id": 1, "source_kind": kind,
                "label": "", "rel_path": "x", "snippet": "", "leaf": "search"}

    try:
        env.cit.issue("alice", "c-web", [row("[doc:aaaaaaaa]", "local"), row("[doc:bbbbbbbb]", "drive"),
                                         row("[doc:cccccccc]", "drive")])
        env.cit.issue("bob", "c-bob", [row("[doc:dddddddd]", "drive")])
