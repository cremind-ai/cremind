"""The ``research`` leaf of the Documentation Search tool, over a faked job layer.

The job runner (:mod:`app.documents.research.jobs`) is replaced by a small
...
import app.documents.research as research_pkg
from app.tools.builtin import cremind_documentation_search as ds
from app.tools.builtin import documentation_search as tool
from app.documents import citations as citations_module
from app.documents import query as query_pkg
...
class FakeJobs:
    """The contract of ``app.documents.research.jobs``, in memory."""
    ...
    def module(self) -> types.ModuleType:
        mod = types.ModuleType("app.documents.research.jobs")
...
def env(monkeypatch, index):
    fake = FakeJobs()
    mod = fake.module()
    monkeypatch.setitem(sys.modules, "app.documents.research.jobs", mod)
...
def _run(profile: str = "alice", **args):
    async def go():
        ...
        try:
            return await tool.DocumentsResearchTool().run({
...
def _read(profile: str = "alice", **args):
    return asyncio.run(tool.DocumentsReadTool().run({"_profile": profile, "_context_id": "ctx-1", **args}))
...
PRELIM = "PRELIMINARY — do not conclude from this; call documentation_search__research again with continue_job={}."
...
@pytest.mark.parametrize("err", [
    ...
    ResearchUnavailable("Documentation search is off for this profile.", status_code="disabled"),
])
...
def test_a_waiting_job_is_shown_with_its_question_and_collected(env):
    ...
    env.monkeypatch.setattr(sys.modules["app.documents.research.jobs"], "continue_job", stay)
...
def test_the_leaf_is_registered_with_its_parameters():
    ...
    from app.documents.research import render as RR

    leaves = {t.name: t for t in tool.get_tools({})}
    assert list(leaves) == ["find_files", "search", "read", "research"]
    leaf = leaves["research"]
    assert make_leaf_name(tool.TOOL_ID, leaf.name) == RR.FN_RESEARCH == "documentation_search__research"
...
def test_the_agent_guidance_now_sends_legal_questions_to_research():
    ...
    group = SimpleNamespace(tool_id="documentation_search", config_name="documentation_search",
                            skills=[SimpleNamespace(name=t.name) for t in tool.get_tools({})])
    guidance = ra._build_documentation_search_guidance([group])
    assert "`documentation_search__research`" in guidance and "MUST go through" in guidance
