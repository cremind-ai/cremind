...
def documents_index_root(system_dir: str) -> str:
    ...
    return os.path.join(os.path.realpath(system_dir), "storage", "documents")
...
def is_documents_index_path(target: str, system_dir: str) -> bool:
    ...
    profile-scoped ``/api/documentation-search`` routes.
    """
    store = os.path.normcase(documents_index_root(system_dir))
