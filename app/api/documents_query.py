"""Documentation search — query API (find / search / read) for the CLI and tests.

``POST /api/documentation-search/query/{find|search|read}`` runs the same leaves the
agent calls (``documentation_search__find_files`` / ``__search`` / ``__read``), with
...
tokens, paging). ``cremind docs search|find|read`` print ``text``.
...
in. ``cremind docs cite`` resolves a token straight from the index.
"""
...
def _tool_llm(profile: str) -> Any:
    ...
    try:
        ...
        group = get_tool_registry().get("documentation_search")
        if group is None:
            return None
...
def get_documents_query_routes() -> List[Route]:
    async def handle_query(request: Request) -> JSONResponse:
        ...
        from app.tools.builtin.documentation_search import execute
    ...
    return [Route("/api/documentation-search/query/{leaf}", handle_query, methods=["POST"])]
