"""User Document Search — deep research jobs, for the CLI and scripts.

``/api/userdocs/research`` starts, follows, answers and cancels the same
research jobs the agent runs through ``user_documents__research`` (see
:mod:`app.userdocs.research.jobs`):

- ``POST /api/userdocs/research``                 — start a job (202).
- ``GET  /api/userdocs/research``                 — this profile's recent jobs.
- ``GET  /api/userdocs/research/{id}?page=&wait=`` — a job, its rendered text
  (a dossier page once finished) and how many pages the dossier has.
- ``POST /api/userdocs/research/{id}/continue``   — answer what the job asked,
  or resume an interrupted one.
- ``POST /api/userdocs/research/{id}/cancel``
...
model group and token budget (the User Documents tool variables, which the
...
:mod:`app.api.userdocs_query` — no citations are registered: there is no
...
from app.userdocs.research.errors import InvalidRequest, JobNotFound, ResearchError
...
def _render_mod() -> Any:
    from app.userdocs.research import render
...
async def _respond(profile: str, view: Any, *, page: int = 1, status: int = 200,
    ...
    if _deliverable(view) and not view.conversation_id:
        ...
        try:
            ...
            logger.debug(f"[userdocs] research job {view.job_id} vanished before its delivery was claimed")
...
def get_userdocs_research_routes() -> List[Route]:
    ...
    return [
        Route("/api/userdocs/research", handle_start, methods=["POST"]),
        Route("/api/userdocs/research", handle_list, methods=["GET"]),
        Route("/api/userdocs/research/{job_id}", handle_get, methods=["GET"]),
        Route("/api/userdocs/research/{job_id}/continue", handle_continue, methods=["POST"]),
        Route("/api/userdocs/research/{job_id}/cancel", handle_cancel, methods=["POST"]),
    ]


__all__ = ["MAX_QUESTION_CHARS", "get_userdocs_research_routes"]
