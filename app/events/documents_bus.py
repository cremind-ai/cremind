"""In-memory pub/sub bus for Documentation search sync-progress snapshots.
...
Every frame is a complete snapshot (see ``app.documents`` progress), so a
...
_instance: DocumentsStreamBus | None = None


def get_documents_stream_bus() -> DocumentsStreamBus:
    global _instance
    if _instance is None:
        _instance = DocumentsStreamBus()
    return _instance


def publish_documents_changed(profile: str | None, snapshot: Dict[str, Any]) -> None:
    """Publish a snapshot for ``profile``. Never raises; ignores a falsy profile."""
    if not profile:
        return
    try:
        get_documents_stream_bus().publish(profile, snapshot)
