...
A User Documents tool prints a token (``[ud:k7m2xq9a#3f9c2e1b]``, see
:mod:`app.userdocs.cite`) next to every passage it returns and records it here
...
it cites (:mod:`app.userdocs.quotes`), so "the contract says '…'" is flagged
...
:func:`app.userdocs.cite.number_tokens`, so "[2]" is the same source everywhere.
...
from app.userdocs import quotes as q
from app.userdocs.cite import (
    TOKEN_RE,
    IssuedCitation,
    locator_label,
    number_tokens,
...
def issue(profile: str, context_id: str | None, citations: list[IssuedCitation]) -> None:
    """Record every token a User Documents tool printed in the conversation
    ...
    try:
        ...
        storage = get_userdocs_citations_storage()
        ...
        logger.warning(f"[userdocs] could not register {len(citations)} citation(s) for {profile}: {exc}")
...
def _bind_pending(profile: str, context_id: str, conversation_id: str) -> None:
    ...
    if ids:
        ...
        get_userdocs_citations_storage().bind_conversation(profile, ids, conversation_id)
...
def purge_profile(profile: str) -> int:
    ...
    return get_userdocs_citations_storage().delete_profile(profile)
...
def profile_index(profile: str) -> Iterator[Any]:
    """The profile's index (an :class:`~app.userdocs.index.IndexDB`) for
    ...
    if rt is not None and rt.db is not None and not rt.db.closed:
        yield rt.db
        return
    uid = rt.uid if rt is not None else get_userdocs_storage().profile_uid(profile)
    ...
    if svc is not None:
        ...
        return
    from app.userdocs.index import IndexDB, IndexDBError

    try:
        db = IndexDB.open(index_path(uid), profile_uid=uid, quick_check=False)
    except IndexDBError as exc:
        logger.warning(f"[userdocs] {profile}: index unreadable for citations ({exc})")
        yield None
        return
    try:
        yield db
...
def _resolve(
    ...
    for row in get_userdocs_citations_storage().rows_for_cite_ids(profile, {c for c, _ in strict.values()}):
        registry.setdefault(row["token"], []).append(row)
...
def finalize_citations(
    ...
    try:
        ...
        logger.exception(f"[userdocs] citation finalization failed for {profile}/{conversation_id}")
        return None
...
def _quote_before(text: str, pos: int) -> str | None:
    for back in range(0, QUOTE_GAP + 1):
        ...
        if ch in _QUOTE_PAIRS:
            ...
            if "ud:" in content.lower() or len(content.strip()) < MIN_QUOTE_CHARS:
                return None
            return content
        if ch.isalnum():
            return None
...
class CitationRenderer:
    ...
    :func:`~app.userdocs.cite.number_tokens` over the whole persisted text,
    ...
    async def render(self, text: str) -> str:
        ...
        try:
            ...
            logger.exception(f"[userdocs] citation rendering failed for {self.profile}/{self.conversation_id}")
            return text
...
def render_plain(
    ...
    try:
        ...
        logger.exception(f"[userdocs] plain citation rendering failed for {profile}/{conversation_id}")
        return text
