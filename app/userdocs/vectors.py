"""The vector side of a User Document Search index: names, payloads, filters.
...
from app.userdocs.embed_prompts import PROMPT_SCHEME
...
# Mirrors app.userdocs.settings.SOURCE_KINDS; kept literal so this module
...
def profile_tag(profile_uid: str) -> str:
    """The 12-hex-char tag a profile's collections start with (after ``ud_``).
...
def collection_name(profile_uid: str, epoch: str, model_key: str, dim: int) -> str:
    """``ud_<uid tag>_<epoch>_<model key>_<dim>_p<PROMPT_SCHEME>``, ≤ 63 chars.
...
def parse_collection_name(name: str) -> tuple[str, str] | None:
    """``(uid tag, epoch)`` of a userdocs collection name, or ``None`` when
    ``name`` is not one. For garbage collection: a ``ud_*`` collection whose
    tag or epoch no live index claims is an orphan."""
    m = re.fullmatch(r"ud_([0-9a-f]{12})_([a-z0-9]+)_.+_\d+_p\d+", name or "")
    if not m:
        return None
