...
_WORKER_MODULE = "app.userdocs.extract.worker"
_TMP_PREFIX = "worker-"
...
def wipe_stale_tmp(tmp_root: str) -> int:
    ...
    for path in stale:
        ...
        removed += 1
    if removed:
        logger.info(f"[userdocs:extract] removed {removed} stale worker temp dir(s) under {tmp_root}")
    return removed
...
class _Worker:
    ...
    def __init__(self, slot: int, proc: subprocess.Popen, tmp_dir: str) -> None:
        ...
        self.expired = False
        self._reader = threading.Thread(target=self._read_replies, name=f"userdocs-extract-w{slot}-out",
                                        daemon=True)
        self._logger = threading.Thread(target=self._forward_stderr, name=f"userdocs-extract-w{slot}-err",
                                        daemon=True)
        self._reader.start()
        self._logger.start()
    ...
    def _forward_stderr(self) -> None:
        try:
            for raw in iter(self.proc.stderr.readline, b""):
                ...
                if line:
                    logger.debug(f"[userdocs:extract:w{self.slot}] {line[:2000]}")
        except (OSError, ValueError):
            pass
...
class ExtractorPool:
    ...
    def extract(self, req: ExtractRequest, timeout_s: float | None = None) -> ExtractResult:
        ...
        try:
            try:
                ...
                logger.warning(f"[userdocs:extract] cannot start an extractor worker: {exc}")
    ...
    def _spawn(self, slot: int) -> _Worker:
        ...
        logger.debug(f"[userdocs:extract] worker {slot + 1} started (pid {worker.pid})")
        return worker
    ...
    def _roundtrip(self, slot: int, worker: _Worker, req: ExtractRequest, timeout_s: float) -> ExtractResult:
        ...
        if reply is _TIMEOUT:
            self._drop(slot, worker)
            logger.warning(f"[userdocs:extract] '{req.name}' ({req.kind}) timed out after {timeout_s:.0f}s")
            return _result(EXTRACT_METADATA_ONLY, req, REASON_TIMEOUT)
        if reply is _EOF:
            code = worker.reap()
            self._drop(slot, worker)
            reason = REASON_OOM if code in (_EXIT_OOM, _SIGKILL) else REASON_CRASH
            logger.warning(f"[userdocs:extract] worker died on '{req.name}' ({req.kind}): exit {code} -> {reason}")
