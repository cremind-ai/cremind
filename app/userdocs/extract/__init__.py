"""Text extraction for User Document Search: out of process, in memory.
...
- :mod:`.dispatch` routes an :class:`~app.userdocs.types.ExtractRequest` to the
  per-format extractor in :mod:`.formats` and turns every failure into an
  :class:`~app.userdocs.types.ExtractResult` with a reason, never an exception.
- :mod:`.worker` is the child's entry point (``python -m
  app.userdocs.extract.worker``). It speaks length-prefixed JSON over its pipes,
