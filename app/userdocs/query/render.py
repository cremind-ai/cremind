...
any ``[ud:`` inside it is defanged, so a file cannot forge a citation or a
block boundary. The renderers also return the tokens they printed, as
:class:`~app.userdocs.cite.IssuedCitation` records for the citation registry,
...
from app.userdocs.cite import IssuedCitation, escape_in_document_text, locator_label, make_token
from app.userdocs.query.filters import DATE_LABELS, type_of_kind
from app.userdocs.query.terms import analyze
...
# ``user_documents`` group); the tool module pins them against the registry.
FN_FIND = "user_documents__find_files"
FN_SEARCH = "user_documents__search"
FN_READ = "user_documents__read"
...
class RenderContext:
    ...
    thumbnail_uri: Callable[[str], str] = lambda fid: f"/api/userdocs/files/{fid}/thumbnail"
...
def _clean(text: str) -> str:
    """Document text made safe to print: a ``[ud:`` inside it can no longer
...
def render_search(outcome: Any, ctx: RenderContext) -> Rendered:
    ...
    tz = _tz_of(outcome)
    head = [f"[User Documents · search · \"{_one_line(_clean(outcome.query))}\" · mode: {outcome.mode} · "
    ...
    def assemble(n: int, expand: bool, chars: int, passages: int) -> tuple[str, _Issued]:
        ...
        if not shown:
            if outcome.total and offset >= outcome.total:
                ...
                summary = "No results. Try other words, fewer filters, or user_documents__find_files by name."
        ...
        body = _search_body(outcome, shown, issued, expand=expand, snippet_chars=chars, passages=passages,
                            tz=tz, offset=offset)
        foot = [
            "Cite every claim with the [ud:…] token printed next to the passage it comes from, copied "
...
def render_find(outcome: Any, ctx: RenderContext) -> Rendered:
    ...
    head = [f"[User Documents · find_files{q} · kind={outcome.kind} · mode: {outcome.mode} · "
            f"{overview_line(outcome.overview)}]"]
    head += _mode_lines(outcome.mode, outcome.mode_reason)
    if outcome.filters:
        head.append("Filters: " + "; ".join(outcome.filters))
...
def render_read(outcome: Any, ctx: RenderContext) -> Rendered:
    ...
    head = [f"[User Documents · read · {_clean(row.get('name') or '')} {ftoken} — " + " · ".join(header_bits) + "]"]
    if outcome.stale:
...
def render_status(message: str, *, code: str | None = None) -> str:
    return f"[User Documents · unavailable{' (' + code + ')' if code else ''}] {message}"
