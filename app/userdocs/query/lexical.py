...
from app.userdocs.index.schema import LEXICAL_FTS5
from app.userdocs.query.filters import Scope
from app.userdocs.query.terms import QueryTerms, build_match, build_phrase_match, like_terms
...
def search_lexical(
    ...
    try:
        ...
        logger.warning(f"[userdocs] keyword search failed: {exc}")
...
def search_like(db: Any, terms: QueryTerms, *, scope: Scope, limit: int) -> LexicalResult:
    ...
    try:
        ...
        logger.warning(f"[userdocs] keyword fallback failed: {exc}")
