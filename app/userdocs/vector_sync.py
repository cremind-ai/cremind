...
def active_collection(rt: Any, emb: Any, store: Any) -> dict[str, Any] | None:
    ...
    from app.userdocs.vectors import PAYLOAD_INDEXES, collection_name
    ...
    if active and active.get("model_key") == model_key and int(active.get("dim") or 0) == dim \
        ...
        if now - rt.last_vector_check > EXISTENCE_CHECK_S:
            ...
            try:
                ...
                logger.debug(f"[userdocs] {rt.profile}: cannot list collections: {exc}")
                return None
            if not present:
                logger.warning(f"[userdocs] {rt.profile}: collection {active['name']} vanished; re-embedding")
    ...
    if active:
        ...
        if active.get("store_key") == skey and active.get("name") != name:
            try:
                ...
                logger.debug(f"[userdocs] {rt.profile}: dropping {active['name']} failed: {exc}")
...
def _payloads(rt: Any, rows: list[dict[str, Any]]) -> list[dict[str, int]]:
    from app.userdocs.vectors import epoch_day, make_payload
...
def step(rt: Any) -> int:
    ...
    try:
        ...
        logger.debug(f"[userdocs] {rt.profile}: collection unavailable: {exc}")
        return 0
    if not coll:
        return 0
    ...
    if todo:
        try:
            ...
            logger.warning(f"[userdocs] {rt.profile}: embedding failed: {exc}")
            return 0
    ...
    try:
        ...
        logger.warning(f"[userdocs] {rt.profile}: vector upsert failed: {exc}")
        return 0
...
def gc(rt: Any) -> int:
    ...
    for _ in range(10_000):
        ...
        if offset is None:
            break
    if removed:
        logger.info(f"[userdocs] {rt.profile}: removed {removed} orphan vectors")
    return removed
