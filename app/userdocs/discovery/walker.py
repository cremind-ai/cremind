...
- **Cloud placeholders are listed, never read** (see
  :func:`app.userdocs.discovery.hashing.is_placeholder`). They come back
...
from app.userdocs.discovery.ignore import CASE_INSENSITIVE, METADATA_ONLY, SKIP, IgnoreMatcher, norm_rel
from app.userdocs.types import FsEntry
...
def _report(on_error: ErrorCallback | None, rel: str, exc: BaseException) -> None:
    logger.debug(f"[userdocs] walk: skipped {rel or '<root>'}: {exc}")
    if on_error is None:
        return
    try:
        on_error(rel, exc)
    except Exception as cb_exc:  # noqa: BLE001 — a reporting hook must not end the walk
        logger.debug(f"[userdocs] walk: on_error callback failed: {cb_exc}")
...
def walk(
    ...
    *,
    ...
    is why :func:`app.userdocs.discovery.scan.scan_diff` asks for one more than
    ...
    :func:`app.userdocs.discovery.hashing.fs_path` so deep Windows paths work.
    """
