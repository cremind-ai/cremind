...
from app.userdocs.discovery.hashing import fs_path, is_icloud_stub, is_placeholder
...
def _parse_line(raw: str) -> _Rule | None:
    ...
    try:
        ...
        logger.debug(f"[userdocs] ignoring malformed ignore pattern {raw!r}")
        return None
...
class IgnoreMatcher:
    ...
    def _load_ignore_file(self, abs_dir: str) -> tuple[_Rule, ...]:
        ...
        try:
            ...
            logger.debug(f"[userdocs] cannot read {path}: {exc}")
            return ()
