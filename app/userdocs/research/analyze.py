...
from app.userdocs.chunking import extract_refs
...
from app.userdocs.research.coverage import coverage_row, is_partial, resolve_scope, unread_reason
...
from app.userdocs.research.windows import (
    WINDOW_TOKENS,
    ChunkRef,
    Window,
    body_chunks,
...
_EVIDENCE_RULE = (
    "Every fact or finding needs evidence: the [ud:…] token printed before the passage, copied exactly, and a "
...
class _Analyze:
    ...
    async def ask(self, *, system: str, user: str, tool: dict[str, Any],
        ...
        for attempt in (1, 2):
            ...
            try:
                ...
                last = exc
                logger.warning(f"[userdocs] research {self.ctx.job_id}: {tool['function']['name']} call failed "
    ...
    async def variants(self, spec: dict[str, Any], query: str) -> list[str]:
        ...
        if llm.raw is None or not llm.can_afford(estimate_tokens(query) + 800, 400):
            return []
        from app.userdocs.query.rerank import query_variants
