...
- **settled** (``complete`` / ``partial``, and ``failed`` / ``cancelled`` with
  ...
  row of the table), read with ``user_documents__read(file='research:<id>',
  page=n)``.

Budget: like the other renderers (:mod:`app.userdocs.query.render`), the
...
with any ``[ud:`` inside it defanged.
"""
...
from app.userdocs.cite import TOKEN_RE, IssuedCitation, escape_in_document_text, make_token, parse_tokens
from app.userdocs.query.render import RenderContext, Rendered, cite_chunk, cite_file, cite_folder
...
LEAF = "research"
FN_RESEARCH = "user_documents__research"
FN_READ = "user_documents__read"

PRELIMINARY = ("PRELIMINARY — do not conclude from this; call user_documents__research again with "
               "continue_job={job_id}.")
...
CITE_RULE = ("Cite each claim with the [ud:…] token printed next to its quote, copied exactly, and tell the "
...
def _header(view: JobView) -> str:
    ...
    return (f"[User Documents · research] job {view.job_id} · {view.status} · {view.mode}/{view.domain} · "
