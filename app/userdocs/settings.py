"""Settings for User Document Search: the admin gate and each profile's own.

Two layers, deliberately in two places:

**The admin gate** (``server_config`` keys ``userdocs.*``) is system-wide
because the embedding model and vector store it rides on are system-wide. It
is written only through ``PUT /api/userdocs/admin`` — never through
``PUT /api/config/embedding``, which always forces a rebuild that refuses every
chat while it runs.

**Each profile's settings** live in the ``userdoc_sources`` table, not in
...
# each lives in ``[userdocs]`` of ``app/config/settings.toml``.
_ADMIN_FIELDS: tuple[tuple[str, str, type, float | None, float | None], ...] = (
    ("userdocs.allowed", "allowed", bool, None, None),
    ("userdocs.storage_budget_mb", "storage_budget_mb", int, 256, 10_000_000),
    ("userdocs.per_profile_budget_mb", "per_profile_budget_mb", int, 0, 10_000_000),
    ("userdocs.vision_daily_cap_default", "vision_daily_cap_default", int, 0, 1_000_000),
    ("userdocs.max_file_mb", "max_file_mb", int, 1, 4096),
    ("userdocs.workers", "workers", int, 1, 8),
    ("userdocs.vector_capacity_mb", "vector_capacity_mb", int, 0, 100_000_000),
    ("userdocs.db_capacity_mb", "db_capacity_mb", int, 0, 100_000_000),
)
...
def default_options() -> dict[str, Any]:
    return {
        ...
        # provider; captioning never runs without it (see app.userdocs vision).
