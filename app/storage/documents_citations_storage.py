"""Sync storage for the Documentation search citation registry.

Schema lives in :mod:`app.storage.models` (``document_citations``); the table is
created by Alembic. The meaning of the rows — what counts as "issued", how an
answer's tokens are verified — lives in :mod:`app.documents.citations`; this
class only reads and writes them.

Synchronous for the same reason as :mod:`app.storage.documents_storage`: the
writers are Documentation Search tools running in ``asyncio.to_thread`` and the
...
from app.storage.models import ConversationModel, DocumentCitationModel

_CIT = DocumentCitationModel.__table__
...
class DocumentCitationsStorage(SyncStorageBase):
    """CRUD for ``document_citations``."""
    ...
    def provider(self) -> DatabaseProvider:
        # Without an explicit provider, follow the rest of Documentation search's main-database state rather than resolving on our own: the
...
_instance: DocumentCitationsStorage | None = None


def get_documents_citations_storage(provider: DatabaseProvider | None = None) -> DocumentCitationsStorage:
    global _instance
    if _instance is None:
        _instance = DocumentCitationsStorage(provider)
    return _instance
