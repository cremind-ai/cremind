...
Schema lives in :mod:`app.storage.models` (``document_research_jobs``); the table
is created by Alembic. What the rows *mean* — the status machine, delivery, the
runner — lives in :mod:`app.documents.research.jobs`; this class only reads and
writes them.

Synchronous for the same reason as :mod:`app.storage.documents_storage`: every
caller runs it through ``asyncio.to_thread`` (the job runner's checkpoints, the
tool, the REST API) or from a Documentation search worker thread (purge).
...
from app.storage.models import DocumentResearchJobModel

_JOBS = DocumentResearchJobModel.__table__

# Kept in step with app.documents.research.types (ACTIVE / WAITING); spelled out
...
def _check(values: dict[str, Any], allowed: frozenset[str] = _WRITABLE) -> None:
    ...
    if unknown:
        raise ValueError(f"unknown document_research_jobs fields: {sorted(unknown)}")


class DocumentResearchStorage(SyncStorageBase):
    """CRUD for ``document_research_jobs``."""
    ...
    def provider(self) -> DatabaseProvider:
        # Follow the rest of Documentation search's main-database state (see
        ...
        return get_documents_storage().provider
...
_instance: DocumentResearchStorage | None = None


def get_documents_research_storage(provider: DatabaseProvider | None = None) -> DocumentResearchStorage:
    global _instance
    if _instance is None:
        _instance = DocumentResearchStorage(provider)
    return _instance
