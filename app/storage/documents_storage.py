"""Sync storage for the main-database half of Documentation search.

Schema lives in :mod:`app.storage.models` (``document_sources``,
``document_captions``, ``document_vision_usage``); the tables are created by
...
:mod:`app.documents.index` and never goes through here.
...
from app.storage.models import (
    DocumentCaptionModel,
    DocumentSourceModel,
    DocumentVisionUsageModel,
)

_SOURCES = DocumentSourceModel.__table__
_CAPTIONS = DocumentCaptionModel.__table__
_USAGE = DocumentVisionUsageModel.__table__
...
class DocumentsStorage(SyncStorageBase):
    """CRUD for the three Documentation search tables in the main database."""
    ...
    def upsert_source(self, profile: str, kind: str, **fields: Any) -> dict[str, Any]:
        ...
        if unknown:
            raise ValueError(f"unknown document_sources fields: {sorted(unknown)}")
        now = _now_ms()
...
_instance: DocumentsStorage | None = None


def get_documents_storage(provider: DatabaseProvider | None = None) -> DocumentsStorage:
    global _instance
    if _instance is None:
        _instance = DocumentsStorage(provider)
    return _instance
