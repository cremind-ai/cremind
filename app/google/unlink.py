...
def _suspend_userdocs_drive(profile: str) -> None:
    """Stop the Drive half of User Document Search from reading through the
...
def _forget(profile: str) -> None:
    ...
    for label, call in (
        ...
        ("userdocs drive", lambda: _suspend_userdocs_drive(profile)),
    ):
        try:
            call()
        except Exception as exc:  # noqa: BLE001
