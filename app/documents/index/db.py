...
with its own schema version (:mod:`app.documents.index.schema`); it is never
...
from app.documents.types import ChunkDiff, ManifestRow, OldChunk
...
def rebuild_file(path: str) -> str | None:
    ...
    logger.warning(f"[documents] index moved aside for rebuild: {path} -> {bak}")
    return bak
...
class IndexDB:
    ...
    def open(cls, path: str, *, profile_uid: str, quick_check: bool | None = None) -> "IndexDB":
        ...
        try:
            ...
            if is_corrupt_error(exc):
                logger.warning(f"[documents] index file is corrupt: {path}: {exc}")
    ...
    def _init(
        ...
        if quick_check and not fresh:
            ...
            if problems != ["ok"]:
                ...
                logger.warning(f"[documents] index quick_check failed: {path}: {detail}")
        ...
        if str(mode).lower() != "wal":
            ...
            logger.warning(f"[documents] index {path} runs in journal_mode={mode}, not WAL")
        ...
        if fresh:
            ...
            try:
                ...
                _rollback(conn)
                raise
            logger.info(f"[documents] created index {path} (epoch {epoch})")
        else:
    ...
    def _validate_and_migrate(conn: sqlite3.Connection, path: str, profile_uid: str) -> str:
        ...
        if stored_uid != profile_uid:
            ...
            logger.warning(f"[documents] index {path} belongs to profile {stored_uid!r}, not {profile_uid!r}")
        ...
        if version < 1 or version > sch.SCHEMA_VERSION:
            logger.warning(
                f"[documents] index {path} has schema {meta.get('schema_version')!r}; "
        ...
        if version < sch.SCHEMA_VERSION:
            ...
            try:
                ...
                _rollback(conn)
                raise
            logger.info(f"[documents] migrated index {path} from schema {version} to {sch.SCHEMA_VERSION}")
        return epoch
    ...
    def _settle_lexical(conn: sqlite3.Connection, path: str) -> str:
        ...
        if sch.fts5_available(conn):
            ...
            if stored == sch.LEXICAL_FTS5:
                logger.warning(f"[documents] full-text objects were missing in {path}; recreated and rebuilt")
            elif stored == sch.LEXICAL_LIKE:
                logger.info(f"[documents] FTS5 is available now; full-text index built for {path}")
            return sch.LEXICAL_FTS5
        conn.execute("BEGIN IMMEDIATE")
        try:
        ...
        if stored != sch.LEXICAL_LIKE:
            logger.warning(f"[documents] SQLite has no FTS5; keyword search for {path} falls back to LIKE")
    ...
    # ── Query-engine reads ─────────────────────────────────────────────────
    #
    # Read-only helpers for :mod:`app.documents.query`. They run on the
