...
from app.documents.discovery.ignore import IGNORE_FILE_NAME, SKIP, IgnoreMatcher, fold_case, norm_rel
...
class _Handler:
    ...
    def dispatch(self, event: Any) -> None:
        try:
            ...
            logger.warning(f"[documents] watcher: dropped an event after an error: {exc}")
...
def _stop_observer(observer: Any) -> None:
    try:
        ...
        logger.debug(f"[documents] watcher: observer stop: {exc}")
...
class SourceWatcher:
    ...
    def start(self) -> None:
        ...
        with self._state_lock:
            ...
            self._stop.clear()
            self._wake.clear()
            thread = threading.Thread(target=self._run, name="documents-watch-settle", daemon=True)
            thread.start()
            self._observer, self._thread = observer, thread
        logger.debug(f"[documents] watcher started on {self.root}")
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                logger.warning(f"[documents] watcher: settle pass failed: {exc}")
                wait_s = 1.0
            finally:
                with self._lock:
                    self._in_hand = 0
    ...
    def _deliver(self, changed: set[str], removed: set[str]) -> None:
        ...
        for i in range(0, max(len(ch), len(rm)), _BATCH):
            ...
            try:
                ...
                logger.warning(f"[documents] watcher: on_paths failed: {exc}")
