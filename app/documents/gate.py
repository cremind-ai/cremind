"""Whether the agent may see the Documentation Search tool in this conversation.
...
def _source_row(profile: str) -> dict[str, Any] | None:
    ...
    return _cached(profile, lambda: get_documents_storage().get_source(profile, uds.SOURCE_LOCAL))
...
def _drive_row(profile: str) -> dict[str, Any] | None:
    ...
    return _cached(_drive_key(profile), lambda: get_documents_storage().get_source(profile, uds.SOURCE_DRIVE))
...
def documents_tool_available(profile: str, origin: str | dict[str, Any] | None = None) -> bool:
    """True when the Documentation Search tool should be offered to ``profile``'s
    ...
    try:
        ...
        logger.debug(f"[documents] tool gate for {profile} failed: {exc}")
        return False
