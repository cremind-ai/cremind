"""The prompt prefixes Documentation search embeds with, and their version.
...
tables) were built without prefixes, so only the documents path applies them:
...
built it. ``PROMPT_SCHEME`` is therefore part of every documents collection name
(``..._p<scheme>``, see :func:`app.documents.vectors.collection_name`). Changing
