...
- each family's edition is chosen by an explicit rule — the user's answer, a
  ...
  (:class:`~app.documents.research.types.Clarification` ``kind="edition"``);
...
from app.documents.chunking import detect_legal_meta
...
from app.documents.research.windows import body_chunks, chunk_tokens_est
...
def _answer_tokens(answers: dict[str, Any]) -> set[str]:
    ...
    for it in items:
        ...
        if not s:
            continue
        m = re.search(r"\[?(?:doc|ud):\s*([0-9a-z]{8})", s, re.IGNORECASE)
