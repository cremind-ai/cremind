...
the server loop, checkpoints to its row in ``document_research_jobs`` after
...
from app.documents.research import context as C
...
from app.documents.research.errors import InvalidRequest, JobNotFound, ResearchBusy, ResearchUnavailable
...
# The tool's variable names (``app.tools.builtin.documentation_search.Var``), spelled
...
def _storage():
    from app.storage.documents_research_storage import get_documents_research_storage

    return get_documents_research_storage()
...
def _open_engine(profile: str) -> Any:
    ...
    if access.engine is None:
        raise ResearchUnavailable(
            access.message or "Documentation search is not available for this profile.",
...
def _pipeline_for(mode: str) -> Callable[[ResearchContext], Awaitable[Dossier]]:
    if mode == MODE_COMPILE:
        from app.documents.research.compile import run_compile
...
def _activity_module() -> Any:
    ...
    try:
        from app.documents.research import activity

        return activity
    except Exception as exc:  # noqa: BLE001
        logger.debug(f"[documents] research activity panel unavailable: {exc}")
        return None
...
def _configured_variables(profile: str) -> dict[str, Any]:
    """The profile's saved Tool Variables for Documentation Search (the adapter
    ...
    try:
        ...
        logger.debug(f"[documents] could not read research settings for {profile}: {exc}")
        return {}
...
def _view(row: dict[str, Any], *, with_dossier: bool = True) -> JobView:
    ...
    if with_dossier:
        try:
            ...
            logger.warning(f"[documents] research job {row['id']}: unreadable dossier ({exc})")
...
def resolve_conversation_id(profile: str, context_id: str | None) -> str | None:
    ...
    try:
        ...
        return get_documents_citations_storage().resolve_conversation(profile, context_id)
    except Exception as exc:  # noqa: BLE001 — a job without a conversation still runs
        logger.warning(f"[documents] could not resolve conversation for {profile}: {exc}")
        return None
    ...
    with _start_lock:
        ...
        for stale in storage.active_for_profile(profile):
            if stale["id"] == run.job_id or stale["id"] in _runs:
                continue
            logger.warning(f"[documents] research job {stale['id']} of {profile} had no runner; interrupted")
...
def _launch(run: _Run, row: dict[str, Any], engine: Any) -> None:
    ...
    run.task = spawn(_runner(run, row, engine), name=f"documents-research:{run.job_id}")
...
async def start_job(
    *,
    ...
    logger.info(f"[documents] research job {job_id} started for {profile} ({spec.mode}/{spec.domain}, "
    ...
    try:
        ...
        logger.warning(f"[documents] research retention prune failed for {profile}: {exc}")
    return _view(row)
...
async def continue_job(
    *,
    ...
    logger.info(f"[documents] research job {job_id} of {profile} resumed from its checkpoint "
...
async def cancel_job(*, profile: str, job_id: str) -> JobView:
    ...
    if run is not None and run.profile == profile:
        ...
        logger.info(f"[documents] research job {job_id} of {profile} cancelled while {row['status']}")
    return await asyncio.to_thread(get_job, profile, job_id)
...
async def cancel_profile_jobs(profile: str, reason: str) -> int:
    ...
    if runs:
        logger.info(f"[documents] stopped {len(runs)} research job(s) of {profile}: {reason}")
    return len(runs)


def signal_cancel(profile: str, reason: str) -> int:
    """:func:`cancel_profile_jobs` for callers on a worker thread (the Documentation
    search engine): schedules the cancellation on the server loop
...
def purge_profile(profile: str) -> int:
    ...
    if removed:
        logger.info(f"[documents] {profile}: deleted {removed} research job(s)")
    return removed
...
async def _open_activity(run: _Run, row: dict[str, Any]) -> tuple[ProgressSink, Any]:
    ...
    try:
        ...
        logger.warning(f"[documents] research job {run.job_id}: activity panel failed to start: {exc}")
        return ProgressSink(), None
...
async def _run_once(run: _Run, row: dict[str, Any], engine: Any) -> None:
    ...
    async def write_usage(sums: dict[str, int], conversation_id: str | None) -> None:
        ...
        try:
            ...
            record = UsageRecord(
                source_kind="documents", tool_id=TOOL_ID, label=f"Document research: {mode}",
            ...
            logger.exception(f"[documents] research job {job_id}: could not record usage")
    ...
    def on_phase_end(_name: str) -> None:
        sums = take_usage()
        if any(sums.values()):
            spawn(write_usage(sums, conv_id), name=f"documents-research-usage:{job_id}")
    ...
    def on_usage(usage: dict[str, int]) -> None:
        ...
        if activity is not None and llm is not None:
            try:
                ...
                logger.debug("[documents] research activity usage update failed", exc_info=True)
    ...
    try:
        ...
        logger.exception(f"[documents] research job {job_id} of {profile} failed")
    ...
    try:
        ...
        logger.exception(f"[documents] research job {job_id}: could not save its result")
...
async def _end_quietly(run: _Run, activity: Any, flush_usage: Callable[..., Awaitable[None]]) -> None:
    ...
    if activity is not None:
        try:
            ...
            logger.debug("[documents] research activity finish failed", exc_info=True)
    logger.info(f"[documents] research job {run.job_id} of {run.profile} stopped: its record was deleted")
...
def _plain_text(view: JobView) -> str:
    lines = [f"[Documentation Search · research] job {view.job_id} · {view.status} · {view.mode}/{view.domain}",
    ...
    lines.append(f"Read the full dossier with documentation_search__research(continue_job=\"{view.job_id}\").")
    return "\n".join(lines)
...
def build_delivery_messages(view: JobView, text: str, *, room: bool = False) -> tuple[str, dict[str, Any]]:
    ...
    call = f'documentation_search__research with continue_job="{jid}"'
...
async def _deliver(row: dict[str, Any]) -> bool:
    ...
    try:
        ...
        if conv is None or conv.get("kind") == "event_run":
            ...
            logger.info(f"[documents] research job {job_id}: conversation {conv_id} cannot take a report; "
                        f"closed out")
            return False
        view = _view(full)
        ...
        if not isinstance(exc, Exception):
            ...
            try:
                ...
                logger.exception(f"[documents] research job {job_id}: could not release its delivery claim")
            raise
        logger.exception(f"[documents] research job {job_id}: delivery to {conv_id} failed; will retry")
        try:
        ...
        if adapter is not None:
            try:
                ...
                logger.exception(f"[documents] research: could not release the forwarder for {conv_id}")
        return False
    logger.info(f"[documents] research job {job_id} ({view.status}) reported to conversation {conv_id}")
    return True
...
async def _deliver_if_idle(conversation_id: str, profile: str) -> None:
    ...
    try:
        ...
        _pending_convs.add(conversation_id)
        logger.exception(f"[documents] research delivery to {conversation_id} failed")
...
async def on_turn_end(*, conversation_id: str, profile: str) -> None:
    ...
    try:
        ...
        _pending_convs.add(conversation_id)
        logger.exception(f"[documents] research turn-end delivery failed for {conversation_id}")
...
async def boot_recover() -> None:
    ...
    try:
        ...
        if ids:
            logger.info(f"[documents] {len(ids)} research job(s) interrupted by the restart: {', '.join(ids)}")
        delivered = await _deliver_owed(None)
        if delivered:
            logger.info(f"[documents] boot: reported {delivered} research result(s)")
    except Exception:  # noqa: BLE001
        logger.exception("[documents] research boot recovery failed")
