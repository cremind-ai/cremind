...
A research model answers with ``{"token": "[doc:…#…]", "quote": "…"}`` pairs.
...
2. the quote occurs in that passage (or straddles into a neighbour), as
   :func:`app.documents.quotes.verify_quote` judges it — exact, normalised
...
from app.documents.quotes import MISMATCH, verify_quote
from app.documents.research.types import Evidence
from app.documents.research.windows import ChunkRef
...
EVIDENCE_SCHEMA: dict[str, Any] = {
    "type": "array",
    "description": (
        "Where this comes from: the [doc:…] token printed before the passage, copied exactly, and "
...
def canonical_token(raw: Any) -> str | None:
    """``raw`` in its canonical ``[doc:<fid>#<c8>]`` form, or None."""
