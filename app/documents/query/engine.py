...
from app.documents.query.lexical import search_lexical, search_like
from app.documents.query.terms import QueryTerms, analyze
from app.documents.query.vector import VectorResult, search_vector
...
def _unavailable_message(snapshot: dict[str, Any]) -> tuple[str, str]:
    ...
    if state == "estimating":
        return "no_index", ("Documentation search is still measuring the folder before its first sync; "
                            "try again shortly.")
    if state == "awaiting_confirmation" and reason == "first_sync":
        return "no_index", ("The first sync is waiting for the user's go-ahead (Settings → My Documents, or "
                            "`cremind docs start`). Nothing is indexed yet.")
    if state == "hold":
        return "no_index", f"Documentation search is on hold ({reason}) and has not indexed anything yet."
...
def _drive_client(profile: str) -> Any:
    ...
    from app.documents.sources import drive as drive_source
...
def _drive_image_loader(client: Any, row: dict[str, Any]) -> Callable[[], bytes | None]:
    ...
    def load() -> bytes | None:
        ...
        try:
            ...
            logger.info(f"[documents] image check: Drive file {row.get('cite_id')} not downloaded: {exc}")
            return None
...
def open_engine(profile: str) -> Access:
    ...
    try:
        ...
        logger.warning(f"[documents] status read failed for {profile}: {exc}")
        return Access(None, "no_engine", "Documentation search status could not be read; try again shortly.")
    ...
    svc = get_service()
    if svc is None:
        return Access(None, "no_engine", "Documentation search is not running on this server.", snapshot)
    ...
    try:
        ...
        logger.warning(f"[documents] {profile}: index could not be opened for search: {exc}")
    ...
    def on_stale(file_id: int) -> None:
        ...
        from app.documents.runtime import P_INTERACTIVE

        try:
            db.mark_dirty([int(file_id)], priority=P_INTERACTIVE)
            svc.wake()
        except Exception as exc:  # noqa: BLE001
            logger.debug(f"[documents] {profile}: could not queue {file_id} for re-indexing: {exc}")
...
class QueryEngine:
    ...
    def vector_handles(self) -> tuple[Any, Any] | None:
        ...
        from app.documents.vector_sync import live_handles
        ...
        try:
            for g in targets:
                if g.file.get("source") == "drive":
                    ...
                    if drive_client is None:
                        try:
                            ...
                            logger.info(f"[documents] {self.profile}: no Drive client for the image check: {exc}")
                            drive_client = False
                ...
                try:
                    ...
                    storage.refund_vision(self.profile, day)
                    logger.info(f"[documents] {self.profile}: image check failed for fid "
    ...
    def _expand(self, hit: Hit) -> None:
        ...
        try:
            ...
            logger.debug(f"[documents] expansion failed for chunk {hit.chunk_id}: {exc}")
    ...
    def read(self, file: str, **kw: Any):
        from app.documents.query.reader import read
