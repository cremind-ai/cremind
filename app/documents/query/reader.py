...
def resolve_file(engine: Any, ref: str) -> tuple[dict[str, Any], str | None]:
    ...
    raw = (ref or "").strip()
    if not raw:
        raise ReadError("MissingFile", "Pass `file`: a [doc:…] token, a file id, a path or a file name.")
    if raw.lower().startswith("research:"):
        raise ReadError("NotAvailable", "Research results cannot be read yet.")
    toks = parse_tokens(raw) if mentions_citation(raw) else []
    ...
    if cite is not None:
        ...
        if db.folder_by_cite(cite) is not None:
            raise ReadError("IsAFolder", "That token names a folder. List its files with "
                            "documentation_search__find_files and filters.folder, then read one of them.")
    ...
    if len(rows) > 1:
        raise ReadError("AmbiguousFile", f"{len(rows)} files are named {raw!r}; pass one of these tokens.",
                        [f"[doc:{r['cite_id']}] {r['rel_path']}" for r in rows[:10]])
    ...
    raise ReadError("NotFound", f"No indexed file matches {raw!r}.",
                    [f"[doc:{r['cite_id']}] {r['rel_path']}" for r in close])
    ...
    *,
    ...
    if asked:
        ...
        if idx is None:
            ...
            selection = f"around [doc:{row['cite_id']}#{c8}]"
    if not body:
