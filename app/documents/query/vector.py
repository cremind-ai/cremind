    ...
    *,
    ...
    if vector is None:
        try:
            ...
            logger.warning(f"[documents] query embedding failed: {exc}")
    ...
    def query(limit: int, filt: Any, *, critical: bool = True) -> list[tuple[int, float]] | None:
        try:
            ...
            logger.warning(f"[documents] vector query on {name} failed: {exc}")
            if critical:
                res.available = False
    ...
    if res.stale_dropped:
        logger.debug(f"[documents] dropped {res.stale_dropped} stale vector hit(s)")
    return res
