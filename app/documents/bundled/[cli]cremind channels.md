...
All `cremind channels` subcommands accept the root-level `--json` flag. It goes right after `cremind`, before the command group (`cremind --json channels list`). A trailing `--json` is rejected — except on `add`, `edit` and `notify-filter`, where a `--json` after the subcommand is that command's own JSON-payload option, not the output switch.
...
```bash
$ cremind --json channels catalog | jq '.telegram.channel.modes[].fields'
{
  "bot_token": {
    "description": "Bot API Token",
    "type": "string",
