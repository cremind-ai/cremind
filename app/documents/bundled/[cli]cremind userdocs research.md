---
description: "Deep research over the user's OWN indexed files with `cremind userdocs research`: `run` a background job that either ANALYZES a question (legal, financial or general: reads the case files in full, searches the law or policy folder from several angles, verifies every quote; `--domain legal` picks the edition of each law in force and stops to ask when it cannot tell) or COMPILES a folder exhaustively (every file read, one table, conflicting values kept side by side — e.g. compile the business results in MKT-report). The result is a dossier: a coverage table of every file read or unread and why, findings with verified [ud:…] citations, gaps. `--follow` prints progress until done; `continue JOB --answer edition=<fid>` answers a clarification; `status --all-pages`, `cancel`, `list`. Not for Cremind's own documentation (that is documentation_search); one quick lookup is `cremind userdocs search`."
---

# `cremind userdocs research` — deep research over your documents
...
kept. It is the job the agent runs through its User Documents tool
(`user_documents__research`); these commands start, follow, answer and cancel
the same jobs from a terminal or a script.

**In a chat, the agent uses the tool, not this command.** Only the tool
registers the answer's `[ud:…]` citations for the conversation (so they
...
covered (`partial`). The budget and the model are the User Documents tool
...
```bash
cremind userdocs research continue 3f9c2e1b7a40 --answer edition=k7m2xq9a --follow
```
...
- **Findings per issue** — each with its provision and verified quotes, every
  quote followed by its `[ud:…]` citation token. Quotes that did not match
...
or `--all-pages`). Check any token with `cremind userdocs cite '[ud:…]'`.
...
(`cremind --json userdocs research status 3f9c2e1b7a40`): it prints the job
(status, progress, the whole dossier) and the rendered text. Progress lines
go to stderr, the result to stdout.

## Subcommands

### `cremind userdocs research run`

**Purpose.** Start a research job.

```bash
cremind userdocs research run QUESTION [--mode analyze|compile] [--domain general|legal|financial]
...
| `--file` | — | Primary scope: a file id or `[ud:…]` token. Repeatable. |
...
### `cremind userdocs research status`

**Purpose.** Show a job: its progress, the question it asked, or its result.

```bash
cremind userdocs research status JOB [--page N | --all-pages] [--follow/-f]
```
...
```bash
$ cremind userdocs research run "Compile the business results: revenue, cost and profit per quarter" \
...
…
[User Documents · research] job 3f9c2e1b7a40 · complete · compile/financial · …
```
...
```bash
cremind userdocs research run "Giải pháp cho tranh chấp đất đai của khách hàng ABC theo Luật Đất đai" \
...
`cremind userdocs research continue 3f9c2e1b7a40 --answer edition=<fid> --follow`.
...
**`UserDocumentsUnavailable`** — User Document Search is off or has no index
yet: `cremind userdocs enable`, then `cremind userdocs status -f`.
...
the budget: `cremind tools set-var user_documents RESEARCH_TOKEN_BUDGET=500000`.
