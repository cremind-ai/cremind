"""Google Drive as a Documentation search source: one profile's Drive half.

A :class:`DriveSource` lives next to the local folder inside a profile's
:class:`~app.documents.runtime.ProfileRuntime` (as ``rt.drive``) and shares its
...
def _exif_time(value: Any) -> str | None:
    try:
        from app.documents.extract.formats.image import exif_datetime
...
def _detect(data: bytes, name: str) -> tuple[str, str | None]:
    try:
        from app.documents.extract.detect import detect_bytes
    except ImportError:
        detect_bytes = None  # type: ignore[assignment]
    if detect_bytes is not None:
        kind, mime = detect_bytes(data, name)
        return kind, mime
    from app.documents.extract.detect import HEAD_BYTES, TAIL_BYTES, sniff
...
def _feature_gate() -> tuple[bool, str | None]:
    try:
        ...
        logger.exception("[documents] could not read the admin gate for Drive")
...
def _purge_citations(profile: str) -> int:
    ...
    return get_documents_citations_storage().delete_source_kind(profile, SOURCE)
...
class DriveSource:
    ...
    def configure(self, row: dict[str, Any] | None) -> None:
        """Apply the drive ``document_sources`` row (or None): turn Drive on or
    ...
    def run_sync(self, *, full: bool = False) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[documents] {self.profile}: Drive sync stopped")
                ...
                if stop.is_set() or self._closed.is_set():
                    ...
                    logger.exception(f"[documents] {self.profile}: Drive sync failed")
    ...
    def _check_account(self, db: Any, ident: str) -> None:
        ...
        if stored and stored != ident:
            ...
            logger.warning(f"[documents] {self.profile}: Drive account changed; re-indexing")
    ...
    def _full_sync(self, client: Any, stop: threading.Event, db: Any, *, first: bool, why: str) -> None:
        ...
        from app.documents.runtime import P_BULK
        ...
        logger.info(
            f"[documents] {self.profile}: Drive reconcile ({why}) listed={len(files)} folders={len(folders)} "
    ...
    def _changes(self, client: Any, stop: threading.Event, db: Any, cursor: str) -> None:
        from app.documents.runtime import P_LIVE
    ...
    def _apply_file(self, db: Any, meta: dict[str, Any], row: dict[str, Any] | None, *, priority: int) -> bool:
        ...
        from app.documents.runtime import P_UPGRADE
        ...
        if row is None:
            try:
                ...
                logger.debug(f"[documents] Drive insert {fid} skipped: {exc}")
                row = db.file_by_drive_id(fid)
                if row is None:
                    return False
        rid = int(row["id"])
    ...
    def _recard(self, db: Any, fid: int) -> bool:
        ...
        from app.documents.chunking import diff_chunks, make_file_card
        from app.documents.runtime import chunks_from_rows, iso_local
    ...
    def _purge_rows(self, db: Any, ids: Iterable[int]) -> int:
        ...
        for fid in list(ids):
            try:
                ...
                logger.debug(f"[documents] Drive purge {fid} failed: {exc}")
        return n
    ...
    def _row_failed(self, db: Any, row: dict[str, Any], name: str, reason: str, error: str, *,
        ...
        from app.documents.runtime import BACKOFF_S, MAX_ATTEMPTS
    ...
    def _source_error(self, exc: BaseException, *, worker: bool) -> None:
        ...
        if kind == "auth_revoked":
            ...
            logger.info(f"[documents] {self.profile}: Drive is rate-limiting; retrying later")
            self._retry_later(HOLD_RETRY_S[1])
        else:
            logger.warning(f"[documents] {self.profile}: Drive sync error ({kind}): {message}")
    ...
    def _enter_hold(self, reason: str, detail: dict[str, Any] | None = None) -> None:
        ...
        if not cur or cur["reason"] != reason:
            logger.warning(f"[documents] {self.profile}: Drive held ({reason})")
    ...
    def _clear_hold(self, *, since: float) -> None:
        ...
        if cur is not None:
            logger.info(f"[documents] {self.profile}: Drive hold ({cur['reason']}) cleared")
    ...
    def _purge_for_hold(self) -> None:
        ...
        logger.warning(f"[documents] {self.profile}: Drive index deleted ({n} files); {what} 7 days ago")
    ...
    def purge_index(self, *, keep_hold: bool = True) -> int:
        ...
        try:
            ...
            if not got:
                logger.warning(f"[documents] {self.profile}: Drive sync did not stop; purging anyway")
            try:
                db = self._db()
                ...
                try:
                    ...
                    logger.exception(f"[documents] {self.profile}: could not delete the Drive citations")
                with self._lock:
    ...
    def _activity(self, kind: str, message: str, *, level: str = "info", detail: Any = None) -> None:
        try:
            ...
            logger.debug(f"[documents] {self.profile}: Drive activity not recorded: {message}")
...
def _iso_ts(value: Any) -> float | None:
    from app.documents.runtime import ts_from_iso
