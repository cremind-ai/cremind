...
def _single_agent_guidance(fns: dict, *, agent: str, extra: str) -> str:
    ...
    return (
        ...
        "codebase\") OR to CREATE, WRITE, REFACTOR, DEBUG, TEST, or RUN code — you "
        f"MUST delegate to `{run_fn}` and MUST NOT inspect, browse, search, or edit "
        "the code yourself with file/shell tools (e.g. list_files, read_file, "
        "exec_shell). Understanding or explaining code COUNTS as a coding task. "
        "First call `change_working_directory` to the relevant project directory if "
        f"it is not already the cwd, then call `{run_fn}` with a complete task brief "
        "(goal, constraints, relevant paths). If it returns status 'running', keep "
