...
from app.cremind_documents.sections import (
    Section,
    SectionSizes,
    children,
    find_section,
...
from app.cremind_documents.sync import DESCRIPTION_MAX_CHARS, SHARED_SCOPE, _clean_name
...
SERVER_NAME = "Cremind Documentation Search"
...
_TOOL_ID = "cremind_documentation_search"
...
class CremindDocumentationSearchTool(BuiltInTool):
...
async def run_doc_search(
    ...
    *,
    ...
    log_label: str = "cremind_documentation_search",
) -> BuiltInToolResult:
    """Shared vector-search + LLM-judge pipeline for documentation-style tools.

    ``cremind_documentation_search`` calls this with the default scopes (shared + the
    ...
    try:
        ...
        # Documentation search batch may be holding. Waiting for it here
...
async def run_read_section(arguments: Dict[str, Any]) -> BuiltInToolResult:
    ...
    """
    tag = "[cremind_documentation_search]"
    profile = arguments.get("_profile") or "admin"
...
def _delivery_budget(profile: str, reserved: str) -> Optional[int]:
    ...
    try:
        ...
        logger.warning(
            "[cremind_documentation_search] could not resolve the agent config for "
...
def _document_reference(
    ...
    try:
        ...
        logger.exception("[cremind_documentation_search] could not build a document reference")
        return name
    ...
    def footer(included: List[Section]) -> str:
        if section_fn is None:
            return (
                ...
                "enabled in Settings → Tools & Skills → Cremind Documentation Search, or "
...
def _judge_unavailable(token_usage: Optional[Dict[str, int]] = None) -> BuiltInToolResult:
    ...
    return BuiltInToolResult(
        structured_content={
            "message": (
                "Cremind documentation search is temporarily unavailable: the relevance "
...
async def _select_best_candidate(
    *,
    ...
    try:
        ...
        logger.exception("[cremind_documentation_search] judge LLM call failed")
        _log("error:judge-llm-failed")
        return None, token_usage, True

    if not function_calls:
        logger.warning(
            "[cremind_documentation_search] judge produced no tool call; "
    ...
    if name == _SELECT_TOOL_NAME:
        ...
        try:
            ...
            logger.warning(
                f"[cremind_documentation_search] judge passed non-integer index: "
                f"{raw_index!r}"
            )
        ...
        logger.warning(
            f"[cremind_documentation_search] judge index {idx} out of range "
    ...
    logger.warning(
        f"[cremind_documentation_search] judge called unknown tool {name!r}; "
        "treating as no-match"
    )
...
def get_tools(config: dict) -> list[BuiltInTool]:
    ...
    return [CremindDocumentationSearchTool(), ReadDocumentationSectionTool()]
