...
class ScheduleCreateTool(BuiltInTool):
    ...
    description: str = (
        ...
        "the `cremind calendar` CLI instead (look up the exact subcommands and "
        "flags first — in the \"cremind calendar\" doc when a Cremind "
        "documentation search is available, otherwise with `cremind calendar "
        "--help` — then run the command with the Shell Executor): e.g. `cremind calendar "
