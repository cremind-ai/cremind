...
cremind-connect (`ATLASSIAN_CLIENT_SECRET`), and the callback URL registered as
`http://127.0.0.1:<port>/` matching `CREMIND_OAUTH_CALLBACK_PORT`.
...
`http://127.0.0.1:<port>/` callback (the pasted URL only supplies the code), so it
matches the Atlassian app regardless of how you reached Cremind.
...
- Linking error about the backend listener → Atlassian needs the fixed-port loopback; run under `cremind serve` and ensure `CREMIND_OAUTH_CALLBACK_PORT` matches the registered callback URL.
