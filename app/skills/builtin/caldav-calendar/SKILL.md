...
Note: events created via this skill's own `create` verb will also be picked up by the listener and emitted as `new_event` on the next poll. This is by design (mirrors imap-email's send-then-see-in-events behavior).
