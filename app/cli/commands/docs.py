...
admin_app = typer.Typer(
    name="admin",
    help="The server-wide Documentation search gate (admin profile).",
    no_args_is_help=True,
)
...
docs_app.add_typer(excludes_app, name="excludes")
docs_app.add_typer(admin_app, name="admin")
docs_app.add_typer(deletions_app, name="deletions")
docs_app.add_typer(research_app, name="research")
docs_app.add_typer(drive_app, name="drive")
...
def _handle_known_errors(e: Any) -> None:
    ...
    if err == "DriveFoldersRequired":
        sys.stderr.write((detail.get("message") or err) + "\n")
        sys.stderr.write("List them: cremind docs drive folders list\n")
        raise typer.Exit(code=1)
...
def _put_settings(ctx: typer.Context, body: dict[str, Any], yes: bool) -> dict[str, Any]:
    ...
    from app.cli.client.docs import put_settings
...
def docker_warning(snap: dict[str, Any]) -> Optional[str]:
    ...
    (``dockerWarning`` in ui/src/utils/documentsView.ts); with the bind mount
...
def documents_status(
    ...
    """Show what Documentation search is doing for this profile."""
    import asyncio

    from app.cli.client._base import Client
    from app.cli.client.docs import get_status, documents_stream_path
    from app.cli.config import Config
    ...
    if follow:
        async def _tail() -> None:
            warned = False
            async with Client(cfg) as client:
                async for event in client.stream(documents_stream_path()):
                    if event.event != "documentation_search":
                        continue
                    if mode.json:
...
@docs_app.command("disable")
@graceful_errors
def documents_disable(
    ctx: typer.Context,
...
def _control(ctx: typer.Context, action: str, *, yes: bool = False, **params: Any) -> dict[str, Any]:
    ...
    from app.cli.client._base import APIError, Client
    from app.cli.client.docs import control
    from app.cli.config import Config
...
def documents_files(
    ...
    from app.cli.client._base import Client
    from app.cli.client.docs import list_files
    from app.cli.config import Config
    from app.cli.output import OutputMode, print_json
...
@docs_app.command("activity")
@graceful_errors
def documents_activity(
    ctx: typer.Context,
    ...
    from app.cli.client._base import Client
    from app.cli.client.docs import activity
    from app.cli.config import Config
    from app.cli.output import OutputMode, print_json
...
def documents_estimate(
    ...
    from app.cli.client._base import Client
    from app.cli.client.docs import get_estimate, start_estimate
    from app.cli.config import Config
...
@docs_app.command("storage")
@graceful_errors
def documents_storage(ctx: typer.Context) -> None:
    """Index size, budget, free disk space, and whether syncing is paused for space."""
    import asyncio

    from app.cli.client._base import Client
    from app.cli.client.docs import get_storage
    from app.cli.config import Config
...
def _query(ctx: typer.Context, leaf: str, body: dict[str, Any]) -> None:
    ...
    from app.cli.client._base import APIError, Client
    from app.cli.client.docs import query
    from app.cli.config import Config
    from app.cli.output import OutputMode, print_json
...
def documents_search(
    ...
    """Search inside your indexed files; every passage comes with a \\[doc:…] citation token."""
    _query(ctx, "search", {
        "query": query,
...
def documents_read(
    ctx: typer.Context,
    file: str = typer.Argument(..., help="A \\[doc:…] token, file id, path inside the indexed folder, or name."),
...
@docs_app.command("cite")
@graceful_errors
def documents_cite(
    ctx: typer.Context,
    token: str = typer.Argument(..., help="A citation token such as \\[doc:k7m2xq9a#3f9c2e1b] (quote it in the shell)."),
) -> None:
    """Show where a \\[doc:…] citation points: the file, the location in it, and the cited text."""
    import asyncio

    from app.cli.client._base import Client
    from app.cli.client.docs import resolve_citations
    from app.cli.config import Config
    from app.cli.output import OutputMode, print_json
    from app.documents.cite import normalize_token, parse_tokens

    raw = token.strip()
    parsed = parse_tokens(raw if raw.startswith(("[", "【")) else f"[{raw}]")
    if not parsed:
        typer.echo("not a citation token — expected [doc:<8 chars>] or [doc:<8 chars>#<8 hex>]", err=True)
        raise typer.Exit(code=1)
...
@docs_app.command("caption")
@graceful_errors
def documents_caption(
    ctx: typer.Context,
...
@docs_app.command("identity")
@graceful_errors
def documents_identity(
    ctx: typer.Context,
...
@docs_app.command("allow-in")
@graceful_errors
def documents_allow_in(
    ctx: typer.Context,
...
def drive_folders_list(
    ...
    from app.cli.client._base import APIError, Client
    from app.cli.client.docs import drive_folders, get_settings
    from app.cli.config import Config
    from app.cli.output import OutputMode, print_json
...
def admin_get(ctx: typer.Context) -> None:
    ...
    from app.cli.client._base import Client
    from app.cli.client.docs import get_admin
    from app.cli.config import Config
...
def admin_set(
    ...
    allow: Optional[bool] = typer.Option(
        None, "--allow/--disallow",
        help="Allow profiles to use Documentation search (needs Vector Embedding on).",
    ),
    ...
    from app.cli.client._base import APIError, Client
    from app.cli.client.docs import put_admin
    from app.cli.config import Config
...
# ── research ───────────────────────────────────────────────────────────────
#
# Deep-research jobs (the agent's `documentation_search__research`) run on the
# server; these commands start, follow, answer and cancel them through
# /api/documentation-search/research. The printed text is what the agent reads, whole.
# The statuses mirror app.documents.research.types, which is not imported
...
def _research_run(
    ...
    *,
    ...
    try:
        ...
        again = f" — `cremind docs research status {jid} --follow` picks it up again" if jid else ""
        ...
        if detail and detail.get("message"):
            ...
            if detail.get("error") == "ResearchBusy" and detail.get("job_id"):
                jid = detail["job_id"]
                sys.stderr.write(f"Follow it: cremind docs research status {jid} --follow\n"
                                 f"Cancel it: cremind docs research cancel {jid}\n")
            raise typer.Exit(code=1) from e
        raise
...
async def _research_follow(
    ...
    *,
    ...
    from app.cli.client.docs import research_get
...
def _research_hint(job: dict[str, Any], pages: Any) -> None:
    ...
    (it says "call documentation_search__research"); this is the terminal's copy."""
    jid = job.get("job_id") or "JOB"
    status = job.get("status") or ""
    if status in _RESEARCH_ACTIVE:
        sys.stderr.write(f"Still running. Follow it: cremind docs research status {jid} --follow\n")
    elif status == "interrupted":
        sys.stderr.write(f"The server restarted during this job. Resume it: "
                         f"cremind docs research continue {jid} --follow\n")
...
def research_run_cmd(
    ...
    async def action(client: Any) -> dict[str, Any]:
        from app.cli.client.docs import research_start
...
def research_continue_cmd(
    ...
    async def action(client: Any) -> dict[str, Any]:
        from app.cli.client.docs import research_continue
...
def research_cancel_cmd(
    ...
    async def action(client: Any) -> dict[str, Any]:
        from app.cli.client.docs import research_cancel
...
def research_list_cmd(
    ...
    async def action(client: Any) -> dict[str, Any]:
        from app.cli.client.docs import research_list
