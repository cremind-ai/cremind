...
class _MdEventHandler(FileSystemEventHandler):
    def __init__(self, scope: str, sync_service: CremindDocumentSyncService):
        super().__init__()
        self._scope = scope
        ...
        with self._lock:
            ...
            def _fire() -> None:
                ...
                try:
                    ...
                    logger.exception(
                        f"[cremind_documents] apply_event failed for {self._scope}/{path_str}"
                    )
...
class CremindDocumentWatcher:
    """Watch one scope's documents directory and forward events."""

    def __init__(self, scope: str, directory: Path, sync_service: CremindDocumentSyncService):
        self._scope = scope
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"[cremind_documents] watcher started for scope={self._scope!r} at {self._directory}"
        )
