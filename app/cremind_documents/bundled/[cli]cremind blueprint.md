---
description: "Package a single profile's **design** into a portable `.cremind-blueprint` file and import it into a profile on another Cremind install — to share a purpose-built agent such as a customer-service assistant. A blueprint carries the persona, per-tool enable/disable and configuration, the LLM provider/model selection, settings changed from defaults, bundled user skills, registered events (schedules, file watchers, skill events) and skill listener processes, but **NO secrets** — API keys, tokens, passwords, OAuth files and `scripts/.env` are never included, and the importer is prompted for them. `export` lists only the components you customized; `import` runs a step-by-step wizard against the current profile (create a fresh profile first to avoid changing an existing one). `upload` sends a blueprint to the Cremind Hub marketplace as a draft through a browser device-code approval, and `install` fetches one from the hub by link or name. Distinct from `cremind backup`, which snapshots the whole system including secrets."
---
