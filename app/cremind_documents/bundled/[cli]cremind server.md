---
description: "Operate and inspect the running Cremind backend with `cremind server`: `health`, `version`, `restart`, `capabilities` and `environment` — is this Cremind running in Docker, native or Kubernetes, on the dev, test or production release channel, with the VNC desktop enabled; plus deployment mode, supervisor, backend version, system paths and the timezone its schedules fire in. On Kubernetes `environment` names the namespace, Helm release, Deployment and Service and prints the `kubectl port-forward` command that reconnects to the pod. It says how to reach the VNC / noVNC remote desktop: a published Docker port, a path on the same origin behind the nginx proxy, or a port-forward plus the URL to open. On Linux x86_64 it reports the CPU model, hypervisor and x86-64 instruction level (`cpu.x86_64_level`, `cpu.missing`) — why the Claude Code CLI hangs at 100% CPU on a qemu64 virtual CPU. Distinct from `cremind version`, which prints the locally installed CLI package."
---
...
| `supervised`      | Whether something restarts the backend when it exits — true whenever `install_mode` above is `docker` or `kubernetes` (the *effective* mode: a Docker container is `docker` even when `INSTALL_MODE` is absent, unknown or says `native`, and a pod is `kubernetes`), under Electron, and on a native install with a boot service (`cremind boot enable`). Also the same value `server environment` prints. |
