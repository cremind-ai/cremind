// Google Drive as the second Documentation search source: what the Drive
...
const view = await load('src/utils/documentsView.ts')
const {
...
test('listDriveFolders: the top without a parent, a folder by encoded id', async () => {
  ...
  env.route('/api/documentation-search/drive/folders', () => json(listing))
  assert.deepEqual(await api.listDriveFolders(URL_, TOKEN, null), listing)
  await api.listDriveFolders(URL_, TOKEN, 'a/b c')
  const urls = env.callsTo('/api/documentation-search/drive/folders').map(c => new URL(c.url))
  assert.equal(urls[0].search, '')
  assert.equal(urls[1].searchParams.get('parent'), 'a/b c')
  assert.equal(env.callsTo('/api/documentation-search/drive/folders')[0].init.headers.Authorization, `Bearer ${TOKEN}`)
})

test('listDriveFolders errors keep their code', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/drive/folders', () => json({ error: 'DriveNotLinked', message: 'Link Google Drive first.' }, 409))
...
test('a Drive settings save sends kind drive with include_folders, and DriveFoldersRequired is thrown', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/settings', (_url, init) => {
  ...
  const err = await api.saveDocumentsSettings(URL_, TOKEN, { kind: 'drive', enabled: true }).then(() => null, e => e)
  assert.equal(err.code, 'DriveFoldersRequired')
  const ok = await api.saveDocumentsSettings(URL_, TOKEN, { kind: 'drive', enabled: true, options: { include_folders: ['A'] } })
  assert.equal(ok.kind, 'done')
  assert.deepEqual(JSON.parse(env.callsTo('/api/documentation-search/settings')[1].init.body),
...
test('turning Drive off comes back as a purge_drive plan to confirm', async () => {
  ...
  const first = await api.saveDocumentsSettings(URL_, TOKEN, { kind: 'drive', enabled: false })
  ...
  assert.deepEqual(env.callsTo('/api/documentation-search/settings').map(c => JSON.parse(c.init.body)),
...
test('Drive control actions carry source=drive; local ones stay as they were', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/control', () => json({ accepted: true, snapshot: { v: 1, boot: 'b', seq: 4, enabled: true, state: 'idle', reason: null } }, 202))
  await api.controlDocuments(URL_, TOKEN, { action: 'sync_now', source: 'drive' })
  await api.controlDocuments(URL_, TOKEN, { action: 'confirm_deletions', source: 'drive' })
  await api.controlDocuments(URL_, TOKEN, { action: 'rescan' })
  assert.deepEqual(env.callsTo('/api/documentation-search/control').map(c => JSON.parse(c.init.body)), [
...
test('the file listing filters by source', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/files', () => json({
  ...
  const page = await api.listDocumentsFiles(URL_, TOKEN, { source: 'drive' })
  assert.equal(page.files[0].source, 'drive')
  assert.equal(new URL(env.callsTo('/api/documentation-search/files')[0].url).searchParams.get('source'), 'drive')
})
