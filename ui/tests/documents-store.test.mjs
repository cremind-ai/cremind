// Which connection the documents store holds, and that nothing crosses profiles.
//
// The profile-events SSE is deliberately opened only on chat routes (each
// origin gets ~6 HTTP/1.1 connections). So Documentation search progress rides
// that stream's `documents` topic on chat routes, opens its own stream only
...
async function setup() {
  ...
  const mod = await load('tests/documents-store-entry.ts')
  mod.setActivePinia(mod.createPinia())
  const settings = mod.useSettingsStore()
  settings.authToken = 'tok-ann'
  const store = mod.useDocumentsStore()
  return { env, mod, settings, store }
}

const subscribers = () => globalThis.__documentsSubscribers
...
test('chat routes subscribe to the profile-events topic, and its frames land', async () => {
  ...
  assert.equal(env.callsTo('/api/documentation-search/status').length, 0)
...
test('other pages poll while the feature is on, and stop once it is off', async () => {
  ...
  env.route('/api/documentation-search/status', () => json(answer))
  store.connect(AGENT)
  store.setRoute('settings')
  assert.equal(store.transport, 'poll')
  await until(() => store.snapshot?.seq === 5, 'first poll')
  assert.equal(env.callsTo('/api/documentation-search/status').at(-1).init.headers.Authorization, 'Bearer tok-ann')
...
test('a page that is not profile-scoped opens nothing', async () => {
  const { env, store } = await setup()
  env.route('/api/documentation-search/status', () => json(snap()))
  store.connect(AGENT)
  store.setRoute('home')
  assert.equal(store.transport, 'none')
  await flush()
  assert.equal(env.callsTo('/api/documentation-search').length, 0)
  store.disconnect()
})

test('My Documents opens its own authenticated stream and parses its frames', async () => {
  const { env, store } = await setup()
  env.route('/api/documentation-search/stream', (_url, init) => {
  ...
  store.setRoute('documents-settings')
  ...
  env.route('/api/documentation-search/status', () => json(snap({ seq: 4 })))
  store.setRoute('settings')
  detach()
  assert.equal(store.transport, 'poll')
  store.disconnect()
...
test('a token change drops the held snapshot and ignores answers for the old token', async () => {
  ...
  const slow = deferred()
  env.route('/api/documentation-search/status', () => slow.promise)
  store.connect(AGENT)
  store.setRoute('settings')
  ...
  env.route('/api/documentation-search/status', () => json(snap({ boot: 'x', seq: 1, state: 'idle', enabled: true })))
  ...
  const auth = env.callsTo('/api/documentation-search/status').map(c => c.init.headers.Authorization)
