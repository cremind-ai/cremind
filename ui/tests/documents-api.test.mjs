// The Documentation search client: the confirm round trip and the error shape.
//
...
test('a plain save resolves to the result, sent once', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/settings', () => json(SAVED))
  const outcome = await api.saveDocumentsSettings(URL_, TOKEN, { kind: 'local', enabled: false })
  assert.equal(outcome.kind, 'done')
  assert.deepEqual(outcome.result, SAVED)
  const calls = env.callsTo('/api/documentation-search/settings')
...
test('ConfirmationRequired comes back as the plan, and confirm() resends the same body with the token', async () => {
  ...
  const first = await api.saveDocumentsSettings(URL_, TOKEN, patch)
  ...
  const bodies = env.callsTo('/api/documentation-search/settings').map(bodyOf)
...
test('a plan that grew while the dialog was open is asked about again', async () => {
  ...
  const first = await api.saveDocumentsSettings(URL_, TOKEN, { kind: 'local', excludes: [] })
  ...
  assert.deepEqual(
    env.callsTo('/api/documentation-search/settings').map(c => bodyOf(c).confirm),
...
test('rebuild goes through the same round trip on /control', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/control', (_url, init) => {
    const body = JSON.parse(init.body)
  ...
  const first = await api.runDocumentsControl(URL_, TOKEN, { action: 'rebuild', reextract: true })
  assert.equal(first.kind, 'confirm')
  const done = await first.confirm()
  assert.equal(done.kind, 'done')
  assert.equal(done.result.accepted, true)
  assert.deepEqual(
    env.callsTo('/api/documentation-search/control').map(bodyOf),
...
test('FeatureNotInstalled from the admin gate carries what to install', async () => {
  env = installBrowser()
  const missing = [{ feature_key: 'documentation_search', extras: ['documents', 'documentation-search'], requires_restart_after_install: false }]
  env.route('/api/documentation-search/admin', () => json({
    error: 'FeatureNotInstalled', missing, message: 'Documentation search requires optional dependencies…',
  }, 409))
  const err = await api.putDocumentsAdmin(URL_, TOKEN, { allowed: true }).then(() => null, e => e)
  assert.equal(err.code, 'FeatureNotInstalled')
  assert.deepEqual(err.missing, missing)
  assert.deepEqual(bodyOf(env.callsTo('/api/documentation-search/admin')[0]), { policy: { allowed: true } })
})

test('an engine that is not running is recognisable', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/files', () => json({ error: 'EngineNotRunning', message: 'Documentation search is not running on this server.' }, 503))
  const err = await api.listDocumentsFiles(URL_, TOKEN).then(() => null, e => e)
...
test('file listing sends the keyset cursor and filters as query parameters', async () => {
  env = installBrowser()
  env.route('/api/documentation-search/files', () => json({ files: [], next: null, counts: {} }))
  await api.listDocumentsFiles(URL_, TOKEN, {
    status: 'error', q: 'report', after: { rel_path: 'a/b.docx', id: 42 }, limit: 100,
  })
  const url = new URL(env.callsTo('/api/documentation-search/files')[0].url)
