/**
 * The pure half of the documents store: how snapshots are folded in, what they
 * mean, and which transport should be carrying them.
 *
 * Kept free of Pinia and Vue so node:test can drive it directly
 * (tests/documents-reducer.test.mjs). The store wraps these in refs.
 *
...
import { isStaleSnapshot, type DocumentsSnapshot } from '../services/documentsApi';

export interface DocumentsStreamState {
...
export const EMPTY_STREAM_STATE: DocumentsStreamState = Object.freeze({
  snapshot: null,
  cursor: null,
}) as DocumentsStreamState;
...
export function reduceSnapshot(
  state: DocumentsStreamState,
  next: DocumentsSnapshot | null | undefined,
): DocumentsStreamState {
...
export function isActive(snap: DocumentsSnapshot | null | undefined): boolean {
...
export function failedCount(snap: DocumentsSnapshot | null | undefined): number {
  if (!snap) return 0;
  if (snap.stages && typeof snap.stages.error === 'number') return snap.stages.error;
  return snap.failed_preview?.length ?? 0;
}

export function needsAttention(snap: DocumentsSnapshot | null | undefined): boolean {
 ...
 */
export function syncProgress(snap: DocumentsSnapshot | null | undefined): SyncProgressModel {
...
export type DocumentsTransport = 'none' | 'profile-events' | 'page-stream' | 'poll';
 ...
 * - a chat route → the `documents` topic of profile-events, which that route
 *   already holds open;
 ...
 */
export function chooseTransport(o: TransportInputs): DocumentsTransport {
