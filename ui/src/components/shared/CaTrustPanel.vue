...
// Local-CA trust panel — shared by the Setup Wizard's "Secure this install"
// step and Settings → HTTPS & Certificate.
//
// Two ways to get the private CA trusted on a device:
//
...
//
// ``variant`` only moves copy between tenses. The wizard runs *before* the
// HTTPS flip and speaks in the future ("the switch will happen without a
// warning"); Settings runs after it and states where the install stands. The
// wizard's restart/pivot explanation stays in the step itself — Settings must
// never tell a running install it is about to restart.
...
const props = withDefaults(defineProps<{
  agentUrl: string;
  tls: TlsStatus | null;
  /** INSTALL_MODE, for deployment-specific copy. */
  installMode?: string | null;
  /** Admin token for ``POST /api/tls/trust``. Omitted in the wizard (the
...
async function runAutoTrust() {
  ...
  try {
    const result = await trustLocalCa(
...
<template>
  <div class="ca-trust-panel">
    ...
    <div v-if="autoSupported" class="section auto-trust">
      ...
      >
        ...
        <template v-if="isSettings">
          This device no longer shows a certificate warning for Cremind.
          Reload the HTTPS address to see it take effect.
        </template>
        <template v-else>
    ...
    <div v-if="isSettings && httpsUrl" class="info-box">
      <strong>The HTTPS address</strong>
      <p>
        <code>{{ httpsUrl }}</code><button
          type="button"
          class="copy-icon-btn"
