...
import {
  browseDocumentsFolders,
  validateDocumentsRoot,
  type BrowseResult,
  type RootCheck,
  type RootMode,
  type DocumentsPolicyView,
  type DocumentsSource,
} from '../../services/documentsApi';

const props = withDefaults(defineProps<{
  source: DocumentsSource;
  policy: DocumentsPolicyView;
  disabled?: boolean;
  saving?: boolean;
...
function scheduleValidate() {
  ...
  validateTimer = setTimeout(async () => {
    ...
    try {
      const result = await validateDocumentsRoot(settingsStore.agentUrl, settingsStore.authToken, candidate);
...
async function browseTo(target: string | null) {
  ...
  browseError.value = '';
  try {
    browse.value = await browseDocumentsFolders(
