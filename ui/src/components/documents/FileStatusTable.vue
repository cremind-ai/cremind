...
const emit = defineEmits<{
  ...
  reindex: [targets: string[], source?: DocumentsSourceKind];
  'engine-down': [];
}>();
...
const source = ref<'' | DocumentsSourceKind>('');
const query = ref('');
const files = ref<DocumentsFileRow[]>([]);
...
async function load(reset: boolean) {
  ...
  loading.value = true;
  try {
    const page = await listDocumentsFiles(settingsStore.agentUrl, settingsStore.authToken, {
    ...
    if (e instanceof DocumentsApiError && e.code === 'EngineNotRunning') {
      emit('engine-down');
      note.value = '';
    } else if (e instanceof DocumentsApiError && e.code === 'NotEnabled') {
...
const SOURCE_OPTIONS: { value: '' | DocumentsSourceKind; label: string }[] = [
...
function isDrive(row: Partial<DocumentsFileRow>): boolean {
  return row.source === 'drive';
}

function reindexRow(row: Partial<DocumentsFileRow>) {
