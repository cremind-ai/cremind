 ...
 * snapshot, worded by `stateBanner` (utils/documentsView.ts), with the actions
...
import type { BannerActionId, StateBanner } from '../../utils/documentsView';
...
<template>
  <section
    ...
    aria-live="polite"
  >
    <div class="doc-banner-icon" aria-hidden="true">
      <Icon v-if="banner.busy" icon="mdi:sync" class="spin" />
      <Icon v-else :icon="icon" />
    </div>
    <div class="doc-banner-main">
      <h2 class="doc-banner-title">{{ banner.title }}</h2>
      <p class="doc-banner-body">{{ banner.body }}</p>
      <div v-if="banner.snippet" class="doc-snippet">
        <code>{{ banner.snippet }}</code>
        <button type="button" class="doc-copy" :title="copied ? 'Copied' : 'Copy'" @click="copySnippet">
          <Icon :icon="copied ? 'mdi:check' : 'mdi:content-copy'" />
        </button>
      </div>
      <div v-if="banner.actions.length" class="doc-banner-actions">
        <ElButton
...
.doc-banner-icon { font-size: 22px; color: var(--tone); line-height: 1; padding-top: 1px; flex: none; }
.doc-banner-main { flex: 1; min-width: 0; }
.doc-banner-title { font-size: 0.95rem; font-weight: 600; margin: 0 0 4px; line-height: 1.4; }
.doc-banner-body { font-size: 0.85rem; color: var(--text-secondary); margin: 0; line-height: 1.5; }
.doc-banner-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }
.doc-banner-actions :deep(.el-button + .el-button) { margin-left: 0; }
...
.doc-copy:hover { color: var(--primary-color); }
