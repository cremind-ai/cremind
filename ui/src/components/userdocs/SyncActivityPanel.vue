...
import { isActive, syncProgress } from '../../stores/userDocsReducer';
import {
  listUserDocsActivity,
  UserDocsApiError,
  type UserDocsActivityEvent,
  type UserDocsSnapshot,
...
const PAGE = 30;
const events = ref<UserDocsActivityEvent[]>([]);
...
function handleError(e: unknown) {
  ...
  if (e instanceof UserDocsApiError && e.code === 'NotEnabled') {
...
async function loadFirstPage() {
  ...
  try {
    const res = await listUserDocsActivity(settingsStore.agentUrl, settingsStore.authToken, { limit: PAGE });
...
async function loadMore() {
  ...
  loadingEvents.value = true;
  try {
    const res = await listUserDocsActivity(settingsStore.agentUrl, settingsStore.authToken, {
...
function eventIcon(e: UserDocsActivityEvent): string {
