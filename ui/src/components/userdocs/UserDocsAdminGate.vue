...
/**
 * The admin gate for User Document Search, on the Vector Embedding page.
 *
 ...
 * Settings → My Documents. The card saves on its own (`PUT /api/userdocs/admin`)
...
import {
  ...
  putUserDocsAdmin,
  UserDocsApiError,
  type UserDocsAdminPolicy,
  type UserDocsAdminView,
  type UserDocsFeatureMissing,
} from '../../services/userdocsApi';

const emit = defineEmits<{
  'feature-missing': [detail: { missing: UserDocsFeatureMissing[]; message: string }];
}>();

const settingsStore = useSettingsStore();
const embeddingStatus = useEmbeddingStatusStore();

const view = ref<UserDocsAdminView | null>(null);
const loadError = ref('');
...
/** The numeric fields, with the server's bounds (app/userdocs/settings.py). */
const NUMBER_FIELDS: {
  key: NumberKey;
  policyKey: keyof UserDocsAdminPolicy;
  label: string;
  min: number;
  max?: number;
  step: number;
...
function toForm(p: UserDocsAdminPolicy): GateForm {
  return {
    allowed: p.allowed,
...
function toPolicy(f: GateForm): UserDocsAdminPolicy {
  return {
    allowed: f.allowed,
...
const patch = computed<Partial<UserDocsAdminPolicy>>(() => {
  ...
  const out: Partial<UserDocsAdminPolicy> = {};
  for (const key of Object.keys(next) as (keyof UserDocsAdminPolicy)[]) {
...
async function reload() {
  loadError.value = '';
  try {
    const v = await getUserDocsAdmin(settingsStore.agentUrl, settingsStore.authToken);
    view.value = v;
    form.value = toForm(v.policy);
    fieldErrors.value = {};
  } catch (e) {
    loadError.value = e instanceof Error ? e.message : 'Could not load the User Document Search settings.';
  }
}
...
async function save(): Promise<boolean> {
  ...
  fieldErrors.value = {};
  try {
    const v = await putUserDocsAdmin(settingsStore.agentUrl, settingsStore.authToken, patch.value);
    view.value = v;
    form.value = toForm(v.policy);
    ElMessage.success('User Document Search settings saved.');
    return true;
  } catch (e) {
...
<template>
  <section class="gate">
    <header class="gate-head">
      ...
      <div>
        <h4 class="gate-title">User Document Search</h4>
        <p class="gate-sub">
    ...
    <template v-else-if="view && form">
      <p v-if="readOnly" class="gate-note">
        ...
        <span>
          Vector Embedding is off, so User Document Search cannot run and these settings are
      ...
      <div class="gate-form">
        <div class="gate-field">
          <label class="gate-label">Allow User Document Search</label>
