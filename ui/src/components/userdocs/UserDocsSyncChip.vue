...
/**
 * The NavRail's User Document Search indicator. Hidden unless there is
...
import { chipTooltip } from '../../utils/userdocsView';
...
const onPage = computed(() => route.name === 'user-documents-settings');

function open() {
  const profile = route.params.profile as string | undefined;
  if (!profile) return;
  void router.push({ name: 'user-documents-settings', params: { profile } });
}
</script>
...
<template>
  <ElTooltip v-if="visible" :content="tooltip" placement="right" :show-after="200">
    <button
      type="button"
      class="rail-item ud-chip"
      :class="[`ud-${mode}`, { active: onPage, 'ud-has-failed': store.isActive && store.needsAttention }]"
      :aria-label="tooltip"
      @click="open"
    >
      <Icon v-if="mode === 'active'" icon="mdi:sync" class="rail-icon ud-spin" />
...
.ud-active { color: var(--primary-color); }
.ud-attention { color: var(--warning-color); }
.ud-has-failed { color: var(--warning-color); }

.ud-spin { animation: ud-spin 1.4s linear infinite; }
@keyframes ud-spin { to { transform: rotate(360deg); } }
@media (prefers-reduced-motion: reduce) { .ud-spin { animation: none; } }
</style>
