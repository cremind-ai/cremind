 ...
 *
 *   POST /api/userdocs/citations/resolve   tokens → verified items (answers
 *                                          saved without citation metadata)
 *   GET  /api/userdocs/files/{fid}         the file record (kind, EXIF, …)
 *   GET  /api/userdocs/files/{fid}/text    the cited passage and its neighbours
 *   GET  /api/userdocs/files/{fid}/raw     the original bytes (local files only)
 *   GET  /api/userdocs/files/{fid}/thumbnail?size=N   an in-memory JPEG
 *
...
async function refusal(res: Response): Promise<UserDocsApiError> {
  const body = await res.json().catch(() => ({}));
  return new UserDocsApiError(res.status, body);
}

function fileUrl(agentUrl: string, fid: string, tail = ''): string {
  return `${resolveBaseUrl(agentUrl)}/api/userdocs/files/${encodeURIComponent(fid)}${tail}`;
}
...
export async function resolveCitations(
  ...
  for (let i = 0; i < tokens.length; i += RESOLVE_BATCH) {
    ...
    const res = await fetch(`${resolveBaseUrl(agentUrl)}/api/userdocs/citations/resolve`, {
      method: 'POST',
...
export interface UserDocFileDetail {
  fid: string;
  name: string;
  rel_path: string;
  kind: string;
...
export async function fetchUserDocFile(
  agentUrl: string,
  authToken: string,
  fid: string,
): Promise<UserDocFileDetail> {
...
export function userDocRawUrl(agentUrl: string, fid: string): string {
  return fileUrl(agentUrl, fid, '/raw');
}

export function userDocThumbnailUrl(agentUrl: string, fid: string, size = 256): string {
 ...
 * (thrown as a UserDocsApiError whose `webLink` says where to open it).
 */
export async function fetchUserDocRaw(
  agentUrl: string,
  authToken: string,
  fid: string,
): Promise<Blob> {
  const res = await fetch(userDocRawUrl(agentUrl, fid), { headers: authHeaders(authToken, false) });
  if (!res.ok) throw await refusal(res);
  return res.blob();
}
