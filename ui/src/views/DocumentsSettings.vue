...
import FileStatusTable from '../components/documents/FileStatusTable.vue';
import FirstSyncEstimateDialog from '../components/documents/FirstSyncEstimateDialog.vue';
import ConfirmPlanDialog from '../components/documents/ConfirmPlanDialog.vue';
import DeletionsConfirmDialog from '../components/documents/DeletionsConfirmDialog.vue';
...
async function loadSettings() {
  try {
    const s = await getDocumentsSettings(settingsStore.agentUrl, settingsStore.authToken);
    applySettings(s);
    loadError.value = '';
  } catch (e) {
...
function applySettings(s: DocumentsSettings) {
  settings.value = s;
  store.setSettings(s);
}
...
async function loadStorage() {
  try {
    storageDetail.value = await getDocumentsStorage(settingsStore.agentUrl, settingsStore.authToken);
  } catch (e) {
    storageDetail.value = null;
    if (e instanceof DocumentsApiError && e.code === 'EngineNotRunning') engineDown.value = true;
  }
}
...
async function saveSource(
  kind: DocumentsSourceKind,
  patch: Omit<DocumentsSettingsPatch, 'kind'>,
  confirmTitle: string,
  confirmLabel: string,
): Promise<boolean> {
  try {
    const outcome = await saveDocumentsSettings(settingsStore.agentUrl, settingsStore.authToken, {
      kind, ...patch,
    });
    const saved: DocumentsSettingsSaved | null = await resolveConfirm(outcome, confirmTitle, confirmLabel);
...
async function control(
  ...
  extra: Omit<DocumentsControlRequest, 'action'> = {},
  confirmText: { title: string; label: string } | null = null,
): Promise<DocumentsControlResult | null> {
  if (acting.value) return null;
  acting.value = true;
  try {
    const outcome = await runDocumentsControl(settingsStore.agentUrl, settingsStore.authToken, {
      action, ...extra,
    });
...
async function reindex(targets: string[], source?: DocumentsSourceKind) {
...
<template>
  <div class="doc-page">
    <div class="doc-container">
      ...
      <div v-if="loading" class="doc-muted">Loading…</div>

      <section v-else-if="loadError" class="doc-card doc-problem">
        <Icon icon="mdi:alert-circle-outline" class="doc-problem-icon" />
        <div>
      ...
      <template v-else-if="settings && local && policy">
        ...
        <template v-else>
          <DocumentsStatePanel :banner="banner" :busy="acting || togglingEnabled" @action="onBannerAction" />
          <DocumentsStatePanel v-if="containerWarning" :banner="containerWarning" />
          ...
          <template v-if="enabled">
            <section v-if="engineDown" class="doc-card doc-problem">
              <Icon icon="mdi:cog-sync-outline" class="doc-problem-icon" />
              <div>
    ...
    <ElDialog v-model="disableOpen" title="Turn off document search?" width="480px" append-to-body>
      <p v-if="driveEnabled" class="doc-dialog-text">
        This turns off your folder only. Google Drive search stays on, with its own index.
      </p>
      <ElRadioGroup v-model="disableDeleteIndex" class="doc-disable-options">
        <ElRadio :value="false">
          <strong>Keep the index</strong>
          <span class="doc-radio-sub">
...
.doc-container { max-width: 860px; margin: 0 auto; display: flex; flex-direction: column; gap: 14px; }
...
.doc-title { font-size: 1.5rem; font-weight: 700; color: var(--text-primary); margin: 0 0 4px; }
.doc-subtitle { color: var(--text-secondary); font-size: 0.875rem; margin: 0; max-width: 680px; line-height: 1.5; }

.doc-card {
  background: var(--surface-color); border: 1px solid var(--border-color);
  border-radius: 10px; padding: 16px 18px;
}
.doc-card h2 { font-size: 1rem; font-weight: 600; color: var(--text-primary); margin: 0 0 6px; }
.doc-card-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; margin-bottom: 12px; }
.doc-card-head h2 { margin: 0; }
.doc-muted { margin: 0 0 12px; font-size: 0.85rem; color: var(--text-secondary); line-height: 1.5; }
.doc-toggle-row { display: flex; align-items: center; justify-content: space-between; gap: 16px; }
.doc-toggle-row .doc-muted { margin: 0; }
...
.doc-problem p { margin: 0 0 10px; font-size: 0.875rem; color: var(--text-secondary); line-height: 1.5; }

.doc-disable-options { display: flex; flex-direction: column; align-items: stretch; gap: 12px; }
.doc-disable-options :deep(.el-radio) {
  height: auto; align-items: flex-start; white-space: normal; margin-right: 0;
}
.doc-disable-options :deep(.el-radio__label) { display: flex; flex-direction: column; gap: 2px; }
.doc-radio-sub { font-weight: 400; font-size: 0.8rem; color: var(--text-secondary); line-height: 1.45; }
.doc-dialog-text { margin: 0 0 12px; font-size: 0.875rem; color: var(--text-secondary); line-height: 1.5; }
</style>
