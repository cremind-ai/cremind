/**
 * What Documentation search says to the user — every banner, label and number
 ...
 * Pure so node:test covers every row (tests/documents-view.test.mjs).
 */

import type {
  DriveFolder,
  PlanEffect,
  DocumentsDriveFolderRef,
  DocumentsDriveLink,
  DocumentsDriveView,
  DocumentsSettings,
  DocumentsSnapshot,
} from '../services/documentsApi';
import { failedCount, isActive, syncProgress } from '../stores/documentsReducer';
...
function rootOf(snap: DocumentsSnapshot, settings: DocumentsSettings | null | undefined): string {
...
function holdBanner(
  ...
  settings: DocumentsSettings | null | undefined,
): StateBanner {
...
export function stateBanner(
  snap: DocumentsSnapshot,
  settings?: DocumentsSettings | null,
  ctx: BannerContext = {},
): StateBanner {
 ...
 * `cremind docs status` prints the same warning as one line.
 */
export function dockerWarning(snap: DocumentsSnapshot | null | undefined): StateBanner | null {
...
export function visionStatus(
  vision: DocumentsSettings['vision'] | null | undefined,
...
export function chipTooltip(snap: DocumentsSnapshot | null | undefined): string {
  if (!snap) return '';
  const failed = failedCount(snap);
 ...
 * covers those.
 */
export function driveBanner(snap: DocumentsSnapshot | null | undefined): StateBanner | null {
...
export function driveCountsText(view: DocumentsDriveView | null | undefined): string {
  const c = view?.counts ?? {};
...
export function driveStatus(view: DocumentsDriveView | null | undefined): DriveStatus | null {
...
export function driveAccessText(link: DocumentsDriveLink | null | undefined): string {
...
export function driveEnableBlocker(
  link: DocumentsDriveLink | null | undefined,
...
export function namedFolders(
  ids: readonly string[],
  refs: readonly DocumentsDriveFolderRef[] | null | undefined = [],
